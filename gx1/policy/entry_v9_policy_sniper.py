"""
ENTRY_V9 Policy SNIPER

SNIPER Entry Policy: EU/London/NY sessions with HIGH volatility allowed.
- EU/OVERLAP/US + (LOW ∪ MEDIUM ∪ HIGH) volatility (enforced by sniper_guard_v1)
- p_long >= min_prob_long (ENTRY_V9 probability threshold)
- Trend features logged but not hard-required (soft gate only)
- Reuses 100% of FARM_V2B infrastructure (model, features, meta-model)

Design Philosophy:
  - Minimal new code: wrapper around FARM_V2B policy logic
  - Different session/vol gates via config-driven guard
  - Same entry model, feature builder, meta-model as FARM
  - Higher frequency than FARM (lower thresholds, more sessions)
"""
...
# Reuse FARM_V2B policy function
...
def apply_entry_v9_policy_sniper(
    ...
    """
    Apply ENTRY_V9 Policy SNIPER - EU/London/NY sessions with HIGH volatility allowed.
    
    This is a wrapper around FARM_V2B policy that uses SNIPER guard instead of FARM guard.
    All scoring, model loading, and feature mapping is reused from FARM_V2B.
    ...
    Args:
        ...
        config: Config dict with "entry_v9_policy_sniper" section
    ...
    require_trend_up = policy_cfg.get("require_trend_up", False)  # Soft gate only in v1
    ...
    logger.info(
        f"[POLICY_SNIPER] Policy params: min_prob_long={min_prob_long}, min_prob_short={min_prob_short}, "
        f"allow_short={allow_short}, allow_high_vol={allow_high_vol}, allow_extreme_vol={allow_extreme_vol}, "
    ...
    # This is the FIRST and ONLY filter - nothing else matters if not EU/OVERLAP/US+(LOW|MEDIUM|HIGH)
    # Uses centralized sniper_guard_v1 for consistency
    # ============================================================================
    ...
    vol_regimes_allowed = "LOW+MEDIUM" + ("+HIGH" if allow_high_vol else "") + ("+EXTREME" if allow_extreme_vol else "")
    logger.info(
        f"[POLICY_SNIPER] SNIPER GUARD: {n_before_guard} -> {n_after_guard} rows passed (EU/OVERLAP/US+{vol_regimes_allowed} only)"
    )
    
    # If no rows remain after guard, reject all
    if len(df) == 0:
        logger.warning(
            "[POLICY_SNIPER] SNIPER GUARD: No rows passed EU/OVERLAP/US+(LOW|MEDIUM|HIGH) filter. Rejecting all signals."
        )
        result_df = df_signals.copy()
        result_df["entry_v9_policy_sniper"] = False
    ...
    if require_trend_up:
        ...
        if trend_col:
            ...
            logger.info(
                f"[POLICY_SNIPER] After TREND_UP filter (soft): {n_before_trend} -> {n_after_trend} rows"
            )
            # In v1: log trend but don't hard-filter (soft gate only)
            # df = df_trend_up  # Commented out for v1 - soft gate only
        else:
            logger.warning("[POLICY_SNIPER] No trend regime column found, skipping trend filter")
    
    # ============================================================================
    # STEP 2: Apply side-aware probability thresholds (p_long OR p_short)
    # Reuse same logic as FARM_V2B
    # ============================================================================
    if "prob_long" not in df.columns:
        raise KeyError("df_signals must have 'prob_long' column for ENTRY_V9_POLICY_SNIPER")
    ...
    if "prob_short" not in df.columns:
        if "p_short" in df.columns:
            ...
            logger.debug("[POLICY_SNIPER] prob_short not found, computed as 1 - prob_long")
    ...
    logger.info(
        f"[POLICY_SNIPER] After side-aware thresholds (long>={min_prob_long}, short>={min_prob_short}): "
        ...
        try:
            ...
            for train_col in meta_feature_cols:
                if train_col not in X_meta.columns:
                    ...
                    logger.warning(f"[POLICY_SNIPER] Feature '{train_col}' missing after mapping, using 0.0")
            ...
            logger.info(
                f"[POLICY_SNIPER] Computed p_profitable for {len(df)} candidates (for logging only): "
                f"mean={p_profitable.mean():.4f}, min={p_profitable.min():.4f}, max={p_profitable.max():.4f}"
            )
        except Exception as e:
            logger.warning(
                f"[POLICY_SNIPER] Failed to compute p_profitable (non-fatal): {e}. "
                f"Continuing without p_profitable logging."
            )
            df["p_profitable"] = 0.0
    else:
        df["p_profitable"] = 0.0
        logger.debug("[POLICY_SNIPER] Meta-model not available - p_profitable not computed")
    ...
    if enable_profitable_filter:
        ...
        if min_prob_profitable > 0.0 and p_profitable_computed:
            ...
            logger.warning(
                f"[POLICY_SNIPER] WARNING: enable_profitable_filter=True is enabled. "
    ...
    # Set policy flag
    df["entry_v9_policy_sniper"] = final_mask.astype(bool)
    
    # Policy score: use max(p_long, p_short) for side-aware scoring
    df["policy_score"] = df[["p_long", "p_short"]].max(axis=1)
    
    # Final coverage
    final_coverage = df["entry_v9_policy_sniper"].sum() / n_original if n_original > 0 else 0.0
    
    # Count signals by side
    n_long_final = (df["entry_v9_policy_sniper"] & (df["_policy_side"].isin(["long", "both"]))).sum()
    n_short_final = (df["entry_v9_policy_sniper"] & (df["_policy_side"].isin(["short", "both"]))).sum()
    
    logger.info(
        f"[POLICY_SNIPER] ===== FINAL RESULT ====="
        ...
        f"n_signals={df['entry_v9_policy_sniper'].sum()}/{n_original} (long={n_long_final}, short={n_short_final})"
    )
    ...
    if len(df) > 0:
        for idx in df.index:
            if idx in result_df.index:
                result_df.loc[idx, "entry_v9_policy_sniper"] = df.loc[idx, "entry_v9_policy_sniper"]
...
def get_entry_policy_v9_sniper():
    """
    Get ENTRY_V9_SNIPER policy function.
    
    Returns:
        Callable that applies SNIPER policy
    """
    return apply_entry_v9_policy_sniper
