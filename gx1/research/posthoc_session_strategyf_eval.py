...
# April-2026 was x10-REPAIRED (2026-06) → NO exclusion anywhere; April is valid data, used in full.
...
def load(csv_path):
    """Read a per-candidate CSV; decode entry_ts/session/year from candidate_uid. Uses ALL data — April-2026
    is x10-REPAIRED (2026-06), so there is NO exclusion. The impossible(>1000bps) count is kept as a data
    sanity tripwire (a healthy chain => 0)."""
    ...
    pc.attrs["raw_n"] = len(pc)
    pc.attrs["impossible"] = int((pc["realized_pnl_bps"].abs() > 1000).sum())
    return pc
...
def summarize(df, label):
    ...
    return dict(
        label=label,
        raw_n=df.attrs.get("raw_n"), impossible=df.attrs.get("impossible"),
        blanket=agg(df),
...
def show(r):
    print(f"\n===== {r['label']} =====")
    print(f"  raw takes={r['raw_n']}  ⚠impossible(>1000bps)={r['impossible']}")
...
def main():
    ...
    # Candidate chain on ALL data — April-2026 is x10-REPAIRED, so NO exclusion.
    new = summarize(load(args.candidate_csv), "CANDIDATE chain")
    show(new)
    if new["impossible"] > 0:
        print(f"  ⚠⚠ {new['impossible']} impossible(>1000bps) trades — data sanity tripwire; investigate.")
    ...
    for tag, path in (("COSTFIX cement (informational)", args.baseline),
        ...
        if not path:
            continue
        try:
            base = summarize(load(path), f"BASELINE — {tag}")
    ...
    print(f"\n=== DECISIVE GATE (clean chain vs LIVE fase2b rollback, ALL data incl. repaired April): "
