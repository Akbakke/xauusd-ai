...
# post-exit regret uses bounded 24-M1-bar follow-through
...
def judge_take(ev: dict, m1: pd.DataFrame, m1_time_arr: np.ndarray) -> dict | None:
    ...
    Verdicts (orthogonal flags, each with a regret quantity in bps):
      ...
      held_too_short  — post-exit bounded follow-through (24 M1 bars)
                        >= 10 bps in trade direction
    ...
    # Post-exit follow-through (held_too_short): exit price = exit_bid for long /
    # exit_ask for short, favorable move over the next 24 M1 bars.
    post_exit_mfe = None
    ...
    # (a profitable take can still have been mistimed).
