...
def _new_candidate_training_progress(
...
def _require_candidate_training_progress(
    value: Mapping[str, Any], *, checkpoint_monitor: str = CHECKPOINT_MONITOR,
) -> dict[str, Any]:
...
def _candidate_entry_learnability_order(
    ...
    if (type(epoch_index) is not int or epoch_index != policy["epoch_index"]
            ...
            or not isinstance(epoch_order, torch.Tensor) or epoch_order.dtype != torch.int64
            or epoch_order.ndim != 1
            or epoch_order.numel() <= policy["batch_offset_end"] * 16):
        raise RuntimeError("[CANDIDATE_ENTRY_LEARNABILITY_REPLAY_SCOPE_INVALID]")
    original = epoch_order.detach().cpu().contiguous()
    if _structured_sha256(original.numpy()) != cohort["epoch_order_sha256"]:
        raise RuntimeError("[CANDIDATE_ENTRY_LEARNABILITY_ORDER_HASH_MISMATCH]")
    selected = torch.cat([original[offset * 16:(offset + 1) * 16]
                          for offset in policy["native_batch_offsets"]])
    if selected.tolist() != cohort["parent_rows"]:
        raise RuntimeError("[CANDIDATE_ENTRY_LEARNABILITY_PARENT_ORDER_MISMATCH]")
    loader_order = original.clone()
    loader_order[policy["batch_offset_start"] * 16:policy["batch_offset_end"] * 16] = selected.repeat(policy["repeats"])
    return loader_order
...
class _CandidateTrainingSession:
    ...
    def checkpoint_monitor(self) -> str:
        policy = self._contract["training"]["checkpoint_policy"]
        monitor = policy["checkpoint_monitor"]
        require_checkpoint_policy(
            policy, context="CANDIDATE_TRAINING_SESSION",
    ...
    def load_checkpoint(self) -> Optional[dict[str, Any]]:
        ...
        state["training_progress"] = _require_candidate_training_progress(
            state["training_progress"], checkpoint_monitor=self.checkpoint_monitor,
        )
    ...
    def save_checkpoint(self, state: Mapping[str, Any]) -> None:
        ...
        value["training_progress"] = _require_candidate_training_progress(
            value["training_progress"], checkpoint_monitor=self.checkpoint_monitor,
        )
        self.validate_top_k_checkpoints(
    ...
    def save_validation_checkpoint(self, *, model: nn.Module, report_only: bool = False) -> dict[str, Any]:
        ...
        if (
            ...
            raise RuntimeError("[CANDIDATE_VALIDATION_CHECKPOINT_PHASE_INVALID]")
        ema = state["weight_ema_state"]
        ...
        if (
            ...
            != canonical_model_state_sha256(model.state_dict())
        ):
            raise RuntimeError("[CANDIDATE_VALIDATION_CHECKPOINT_EMA_INVALID]")
        buffers = {name: value for name, value in online.items() if name not in parameter_names}
        if any(not torch.equal(ema["shadow"][name], value) for name, value in buffers.items()):
            raise RuntimeError("[CANDIDATE_VALIDATION_CHECKPOINT_BUFFER_DRIFT]")
        payload = {
            "schema_version": "gx1_candidate_validation_checkpoint_v1",
            "session_contract_path": str(self._contract_path),
            "session_contract_file_sha256": _sha256_file(self._contract_path),
            "session_contract_sha256": self._contract_sha256,
            "epoch_index": int(state["epoch_index"]),
            "global_optimizer_steps": int(state["global_optimizer_steps"]),
            "model_variant": "weight_ema",
            "model_state": dict(ema["shadow"]),
            "target_model_state": dict(target),
            "online_model_state_sha256": canonical_model_state_sha256(online),
            "model_state_sha256": canonical_model_state_sha256(ema["shadow"]),
            "target_model_state_sha256": canonical_model_state_sha256(target),
            "online_buffers": buffers,
        ...
        if directory.exists():
            ...
            if directory.is_symlink() or not directory.is_dir() or info.st_uid != os.getuid() or info.st_mode & 0o077:
                raise RuntimeError("[CANDIDATE_VALIDATION_CHECKPOINT_DIRECTORY_INVALID]")
        else:
        ...
        if destination.exists() or destination.is_symlink():
            if destination.is_symlink() or not destination.is_file():
                raise RuntimeError("[CANDIDATE_VALIDATION_CHECKPOINT_COLLISION]")
            existing = torch.load(destination, map_location="cpu", weights_only=True)
            if (
                not isinstance(existing, Mapping) or set(existing) != set(payload)
                or any(existing[key] != payload[key] for key in scalar_keys)
                or canonical_model_state_sha256(existing["model_state"]) != payload["model_state_sha256"]
                or canonical_model_state_sha256(existing["target_model_state"]) != payload["target_model_state_sha256"]
                or set(existing["online_buffers"]) != set(buffers)
                or any(not torch.equal(existing["online_buffers"][key], value) for key, value in buffers.items())
            ):
                raise RuntimeError("[CANDIDATE_VALIDATION_CHECKPOINT_COLLISION]")
        else:
            ...
            try:
                ...
                _fsync_regular_file(Path(temporary))
        ...
        return {
            ...
            "global_optimizer_steps": payload["global_optimizer_steps"],
        }
...
class EntryV10CtxDataset(Dataset):
    ...
    def set_unified_exit_lifecycle_v2_epoch(self, epoch_index: int) -> None:
        if self._unified_exit_lifecycle_v2 is None:
            raise RuntimeError("UNIFIED_EXIT_LIFECYCLE_V2_NOT_BOUND")
        self._unified_exit_lifecycle_v2.set_epoch_index(int(epoch_index))
...
def _native_val_gate_health_stats(result: Mapping[str, Any]) -> dict[str, Any]:
    """Apply the existing candidate gate rules to native full-VAL observations.

    Native rollout records each shared state once while either Exit side is active.
    Entry records the complete
    Entry cohort. Neither aggregate gate variance nor positive net Bps replaces
    the per-feature liveness and saturation checks.
    """

    stats: dict[str, Any] = {}
    for surface, widths, shape in (
        ("entry", _MODEL_NATIVE_COOPERATION_GATE_WIDTHS, _MODEL_NATIVE_FEATURE_TF_GATE_SHAPE),
        ("exit", _UNIFIED_EXIT_COOPERATION_GATE_WIDTHS, _UNIFIED_EXIT_FEATURE_TF_GATE_SHAPE),
    ):
        evidence = result.get(f"{surface}_gate_and_feature_route_diagnostics")
        if not isinstance(evidence, Mapping) or not isinstance(evidence.get("routes"), Mapping):
            raise RuntimeError("[CANDIDATE_NATIVE_VAL_GATE_EVIDENCE_MISSING]")
        routes = evidence["routes"]
        raw_stats: dict[str, Any] = {}
        for name, width in {**widths, "family_tf_feature_gate": int(np.prod(shape))}.items():
            raw = routes.get(f"exit_{name}")
            if not isinstance(raw, Mapping):
                raise RuntimeError("[CANDIDATE_NATIVE_VAL_GATE_EVIDENCE_MISSING]")
            expected_unit = "entry_row" if surface == "entry" else "active_entry_state_shared_by_sides"
            if raw.get("observation_unit") != expected_unit:
                raise RuntimeError("[CANDIDATE_NATIVE_VAL_GATE_OBSERVATION_UNIT_INVALID]")
            rows = raw.get("batch_row_count")
            if type(rows) is not int or rows <= 0:
                raise RuntimeError("[CANDIDATE_NATIVE_VAL_GATE_ROWS_INVALID]")
            if name == "family_tf_feature_gate" and tuple(raw.get("shape_tail", ())) != tuple(shape):
                raise RuntimeError("[CANDIDATE_NATIVE_VAL_GATE_SHAPE_INVALID]")
            raw_stats[f"{name}_rows"] = rows
            for field in ("mean_weight", "std_weight", "min_observed", "max_observed"):
                values = np.asarray(raw.get(f"coordinate_{field}", ()), dtype=np.float64)
                if values.shape != (width,) or not np.isfinite(values).all():
                    raise RuntimeError("[CANDIDATE_NATIVE_VAL_GATE_COORDINATES_INVALID]")
                raw_stats[f"{name}_{field}"] = values.tolist()
            if name == "family_tf_feature_gate":
                raw_stats[f"{name}_min_std"] = min(raw_stats[f"{name}_std_weight"])
            else:
                entropy = raw.get("raw_entropy_mean")
                if isinstance(entropy, bool) or not isinstance(entropy, (int, float)) or not math.isfinite(entropy):
                    raise RuntimeError("[CANDIDATE_NATIVE_VAL_GATE_ENTROPY_INVALID]")
                raw_stats[f"{name}_entropy_mean"] = float(entropy)
                raw_stats[f"{name}_min_mean"] = min(raw_stats[f"{name}_mean_weight"])
        failures = _cooperation_gate_health_failures(
            raw_stats, gate_widths=widths, feature_gate_shape=shape,
        )
        prefix = "exit_" if surface == "exit" else ""
        stats.update({f"{prefix}{key}": value for key, value in raw_stats.items()})
        stats[f"{prefix}cooperation_gate_health_ok"] = not failures
        stats[f"{prefix}cooperation_gate_health_failures"] = failures
        if surface == "exit":
            stats["exit_gate_evidence_schema_version"] = UNIFIED_EXIT_GATE_EVIDENCE_SCHEMA_VERSION
            blocking, provisional = _candidate_static_exit_feature_gate_disposition(failures)
            if provisional is not None:
                stats["candidate_exit_static_feature_gate_diagnostic"] = provisional
                stats["candidate_exit_gate_health_provisional_ok"] = not blocking
    return stats
...
def _episode_native_exit_train_v2(
    *,
    ...
    """Consume canonical sampled random-access v2 states as one outer batch."""

    adapter = getattr(dataset, "_unified_exit_lifecycle_v2", None)
    materialize = getattr(
        adapter, "materialize_random_access_training_item_v1", None
    )
    binding_owner = getattr(adapter, "random_access_training_bindings_v1", None)
    if not callable(materialize) or not callable(binding_owner):
        raise RuntimeError(
            "[UNIFIED_EXIT_RANDOM_ACCESS_V2_ADAPTER_BOUNDARY_REQUIRED]"
        )
    bindings = binding_owner()
    required_bindings = {
        "sampler_contract",
    ...
    rows = [int(value) for value in entry_row_indices.detach().cpu().tolist()]
    raw_items = [
        materialize(row, outer_batch_index=position)
        for position, row in enumerate(rows)
    ]
    items = [item for item in raw_items if item is not None]
    if not items:
        entry_targets = torch.zeros(
            (len(rows), 3),
            dtype=entry_decision_representations.dtype,
            device=device,
        )
        entry_valid = torch.zeros(
            (len(rows), 3), dtype=torch.bool, device=device
        )
        entry_valid[:, 2] = True
        return (
    ...
    batch = collate_random_access_training_items(
        ...
        sampler_contract=bindings["sampler_contract"],
        ...
        expected_market_closure_authority_sha256=(
            bindings["market_closure_authority_sha256"]
        ),
        expected_economic_step_manifest_sha256=(
            bindings["economic_step_manifest_sha256"]
        ),
    ...
    _episode_stats_update(
        stats,
        q_values=outcome["prediction"],
        targets=outcome["targets"],
        valid=outcome["valid_mask"],
    )
    stats["eligible_entry_rows"] = int(batch["selected_entry_count"])
    stats["random_access_transition_count"] = int(
        batch["transition_count"]
    )
    stats["random_access_online_forward_calls"] = int(
        outcome["online_forward_calls"]
    )
    stats["random_access_target_forward_calls"] = int(
    ...
    return (
        ...
        outcome["entry_targets"],
        outcome["entry_valid_mask"],
    )
...
def _restore_candidate_training_checkpoint(
    ...
    *,
    ...
    entry_learnability_origin: Optional[Mapping[str, Any]] = None,
) -> dict[str, Any]:
    ...
    if (session.directory / ENTRY_LEARNABILITY_RECEIPT_NAME).exists() and entry_learnability_origin is None:
        raise RuntimeError("[CANDIDATE_ENTRY_LEARNABILITY_PRODUCTION_RESUME_FORBIDDEN]")
    if entry_learnability_origin is not None:
        ...
        if (not isinstance(history, Mapping)
                or Path(history["transition_receipt"]["path"]) != session.directory / ENTRY_LEARNABILITY_RECEIPT_NAME):
            raise RuntimeError("[CANDIDATE_ENTRY_LEARNABILITY_RECEIPT_MISSING]")
        receipt_binding = history["transition_receipt"]
        receipt = read_bound_json(Path(receipt_binding["path"]), receipt_binding["sha256"])
        if receipt.get("origin") != origin:
            raise RuntimeError("[CANDIDATE_ENTRY_LEARNABILITY_RECEIPT_ORIGIN_MISMATCH]")
...
def _candidate_training_epoch_order(
    ...
    if (
        ...
        raise RuntimeError("[CANDIDATE_RANDOM_ACCESS_FULL_TRAIN_COVERAGE_INVALID]")
    schedule = adapter.set_full_population_epoch_index(int(epoch_index))
    ...
    parent_by_child = dict(zip(children.tolist(), parents.tolist()))
    if train_ds._random_access_child_index_by_parent != dict(
...
def _native_candidate_val_context_binding(context: Mapping[str, Any]) -> dict[str, Any]:
    """Bind native VAL sources and compute limits into the durable session."""
    ...
    factory = context["state_factory"]
    if not isinstance(factory, RandomAccessValStateFactoryV1):
        raise RuntimeError("[CANDIDATE_NATIVE_VAL_FACTORY_INVALID]")
    frame = context["frame"]
...
def _native_candidate_epoch_validation(
    *, session: _CandidateTrainingSession, model: nn.Module, target_model: nn.Module,
    ...
    try:
        ...
        with weight_ema.evaluating(model):
            ...
            return evaluate_bound_full_val_v1(
                model=model, entry_dataset=val_ds, frame=context["frame"],
...
def _native_candidate_validation_stats(result: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(heads, Mapping) or heads.get("target_model_state_sha256") != result["checkpoint_binding"]["target_model_state_sha256"]:
        raise RuntimeError("[CANDIDATE_NATIVE_VAL_ACTIVE_HEAD_EVIDENCE_INVALID]")
    stats = {
        **heads, **_native_val_gate_health_stats(result),
        "entry_exit_policy_metrics": dict(result["entry_exit_policy_metrics"]),
...
def _load_candidate_optimizer_procedure_successor_state(
    *, session: _CandidateTrainingSession, origin: Mapping[str, Any],
    ...
    if not (continuation or target_refresh or entry_learnability) and "gx1/models/entry_v10/entry_v10_ctx_train_v3.py" not in changed_sources:
    ...
    *,
    ...
    if restored_state is None:
        ...
        restored = _restore_candidate_training_checkpoint(
            ...
            entry_learnability_origin=(candidate_resume_origin if candidate_resume_origin is not None
                and candidate_resume_origin.get("schema_version") == "gx1_candidate_entry_learnability_origin_v1" else None),
        )
        ...
        progress = _require_candidate_training_progress(
            restored["training_progress"], checkpoint_monitor=checkpoint_monitor,
        )
    ...
    def _pause_if_due(
        *, phase_value: str, epoch_value: int, batch_offset_value: int,
        ...
        native_val_pause: Optional[Mapping[str, Any]] = None,
    ) -> None:
        if execution_budget is None:
            return
        reason = reason_override or candidate_execution_pause_reason(
            execution_budget,
        ...
        if reason is not None:
            ...
            if native_val_pause is not None:
                evidence["native_val_pause"] = dict(native_val_pause)
        ...
        session.save_checkpoint(
            {
                ...
                "rng_state": _attended_session_rng_state(device=device),
                "training_progress": _require_candidate_training_progress(
                    progress, checkpoint_monitor=checkpoint_monitor
                ),
    ...
    def _result() -> dict[str, Any]:
        checked = _require_candidate_training_progress(
            progress, checkpoint_monitor=checkpoint_monitor
        )
    ...
    while True:
        ...
        if phase == "train":
            ...
            if next_batch_offset == expected_train_batches:
                ...
                if native_val_context is not None:
                    # Full VAL gets its own guarded invocation; its fixed wall
                    # contract cannot inherit an almost-spent TRAIN window.
                    _pause_if_due(
                        phase_value=phase, epoch_value=epoch_index,
                        batch_offset_value=0,
                        reason_override="native_full_val_phase_boundary",
                    )
                continue
            train_ds.set_unified_exit_lifecycle_v2_epoch(int(epoch_index))
            loader_order = epoch_order
            if (candidate_resume_origin is not None and candidate_resume_origin.get("schema_version")
                    == "gx1_candidate_entry_learnability_origin_v1"):
                from gx1.contracts.local_random_access_campaign_v2 import read_bound_json
                from gx1.contracts.unified_exit_native_candidate_campaign_v1 import (
                ...
                if (not isinstance(random_access, Mapping) or adapter._epoch_index != 1
                        or random_access["full_population_schedule"]["epoch_index"] != 1
                        or [train_ds._random_access_child_index_by_parent[parent] for parent in cohort["parent_rows"]]
                           != cohort["child_rows"]):
                    raise RuntimeError("[CANDIDATE_ENTRY_LEARNABILITY_NATIVE_MAPPING_INVALID]")
                sample_plan = {str(row): {"samples": random_access["samples_by_entry"][row],
                                         "anchor": random_access["anchors_by_entry"][row]}
                               for row in cohort["child_rows"]}
        ...
        if native_val_context is not None:
            if next_batch_offset != 0 or progress["validation_snapshot"] is not None:
                raise RuntimeError("[CANDIDATE_NATIVE_VAL_LEGACY_PROGRESS_INVALID]")
            if native_val_context["max_wall_seconds"] >= execution_budget["max_invocation_seconds"]:
                raise RuntimeError("[CANDIDATE_NATIVE_VAL_INVOCATION_TOO_SHORT]")
            remaining = execution_budget["max_invocation_seconds"] - (time.monotonic() - invocation_started_monotonic)
            if remaining < native_val_context["max_wall_seconds"] + 60:
                _pause_if_due(
                    phase_value="validation", epoch_value=epoch_index, batch_offset_value=0,
                    reason_override="native_full_val_fresh_window_required",
                )
            native_result = _native_candidate_epoch_validation(
                session=session, model=model, target_model=target_model,
                weight_ema=weight_ema, val_ds=val_ds, device=device,
                batch_size=batch_size, epoch_index=epoch_index, context=native_val_context,
            )
            if native_result.get("decision") == "PAUSED_RESUMABLE":
                rollout_path = session.directory / "native_val" / f"epoch_{epoch_index + 1:04d}" / "ROLLOUT_PROGRESS.json"
                _pause_if_due(
                    phase_value="validation", epoch_value=epoch_index, batch_offset_value=0,
            ...
            acc = float(val_stats["entry_unique_target_action_agreement"])
        else:
        ...
        policy_pnl = (
            checkpoint_metric(val_stats, checkpoint_monitor=checkpoint_monitor)
            if metric_available else None
        )
        if not metric_available:
            log.info("[CANDIDATE_CHECKPOINT_INELIGIBLE] epoch=%d reason=%s",
                     int(epoch_index) + 1, val_stats["checkpoint_selection_unavailable_reason"])
        improved = bool(
            policy_pnl is not None and np.isfinite(policy_pnl)
            and (
        ...
        if admission_ok and policy_pnl is not None and np.isfinite(policy_pnl):
...
def load_completed_candidate_epoch_for_seal(
    *,
    ...
    progress = _require_candidate_training_progress(
        state["training_progress"], checkpoint_monitor=session.checkpoint_monitor
    )
...
def run_train(
    ...
    unified_exit_v2_root_manifest_path: Optional[Path] = None,
    train_unified_exit_v2_compact_path: Optional[Path] = None,
    train_unified_exit_v2_manifest_path: Optional[Path] = None,
    val_unified_exit_v2_compact_path: Optional[Path] = None,
    val_unified_exit_v2_manifest_path: Optional[Path] = None,
    unified_exit_v2_economics_readiness_path: Optional[Path] = None,
    train_unified_exit_v2_cost_authority_path: Optional[Path] = None,
    val_unified_exit_v2_cost_authority_path: Optional[Path] = None,
    unified_exit_v2_preflight_only: bool = False,
) -> Optional[dict[str, Any]]:
    from gx1.contracts.entry_model_native_train_launch_v1 import (
        require_training_recipe_source_provenance_metadata,
    )
    ...
    v2_artifacts = (
        unified_exit_v2_root_manifest_path,
        train_unified_exit_v2_compact_path,
        train_unified_exit_v2_manifest_path,
        val_unified_exit_v2_compact_path,
        val_unified_exit_v2_manifest_path,
        unified_exit_v2_economics_readiness_path,
        train_unified_exit_v2_cost_authority_path,
        val_unified_exit_v2_cost_authority_path,
    )
    if any(value is None for value in v2_artifacts):
        raise RuntimeError("[UNIFIED_EXIT_LIFECYCLE_V2_ARTIFACTS_REQUIRED]")
    ...
    unified_exit_v2_wiring = bind_canonical_unified_exit_lifecycle_v2(
        dataset_run_id=str(dataset_run_id),
        root_manifest_path=Path(unified_exit_v2_root_manifest_path),
        compact_paths={
            "train": Path(train_unified_exit_v2_compact_path),
            "val": Path(val_unified_exit_v2_compact_path),
        },
        compact_manifest_paths={
            "train": Path(train_unified_exit_v2_manifest_path),
            "val": Path(val_unified_exit_v2_manifest_path),
        },
        economics_readiness_path=Path(unified_exit_v2_economics_readiness_path),
        cost_authority_paths={
            "train": Path(train_unified_exit_v2_cost_authority_path),
            "val": Path(val_unified_exit_v2_cost_authority_path),
        },
        datasets={"train": train_ds, "val": val_ds},
        feature_source_owners={
            "train": train_exit_lifecycle,
            "val": unified_exit_lifecycle.splits["val"],
        },
    )
    log.info(
        "[UNIFIED_EXIT_LIFECYCLE_V2_ACTIVE] root=%s train_rows=%d "
        "val_rows=%d legacy_exit_target_rejected=1",
        unified_exit_v2_wiring["root_manifest_sha256"],
        unified_exit_v2_wiring["splits"]["train"]["compact_rows"],
        unified_exit_v2_wiring["splits"]["val"]["compact_rows"],
    )
    if unified_exit_v2_preflight_only:
        return unified_exit_v2_wiring
    ...
    unified_exit_lifecycle_evidence = {
        **unified_exit_v2_wiring,
        "root_manifest_sha256": unified_exit_v2_wiring["root_manifest_sha256"],
        "splits": {
            split: {
                **unified_exit_v2_wiring["splits"][split],
                "lifecycle_manifest_sha256": unified_exit_v2_wiring["splits"][split][
                    "compact_manifest_sha256"
                ],
            }
            for split in ("train", "val")
        },
        "feature_source_root_manifest_sha256": (
            unified_exit_feature_source_evidence["root_manifest_sha256"]
        ),
    }
    log.info(
        "[UNIFIED_EXIT_LIFECYCLE_V2_BOUND] manifest_sha256=%s "
        "train_compact_rows=%s val_compact_rows=%s",
        unified_exit_lifecycle_evidence["root_manifest_sha256"],
        unified_exit_lifecycle_evidence["splits"]["train"]["compact_rows"],
        unified_exit_lifecycle_evidence["splits"]["val"]["compact_rows"],
    )
    ...
    if attended_session is not None:
        ...
        if attended_start_offset >= _expected_train_batches:
            raise RuntimeError("[ATTENDED_RESEARCH_CHECKPOINT_OFFSET_INVALID]")
        train_ds.set_unified_exit_lifecycle_v2_epoch(0)
    ...
    for epoch in range(epochs):
        if candidate_training_already_completed:
            break
        last_epoch = epoch + 1
        train_ds.set_unified_exit_lifecycle_v2_epoch(int(epoch))
...
def main() -> None:
    ...
    parser.add_argument(
        "--unified-exit-lifecycle-v2-root-manifest-json", type=Path, required=True
    )
    parser.add_argument(
        "--train-unified-exit-lifecycle-v2-parquet", type=Path, required=True
    )
    parser.add_argument(
        "--train-unified-exit-lifecycle-v2-manifest-json", type=Path, required=True
    )
    parser.add_argument(
        "--val-unified-exit-lifecycle-v2-parquet", type=Path, required=True
    )
    parser.add_argument(
        "--val-unified-exit-lifecycle-v2-manifest-json", type=Path, required=True
    )
    parser.add_argument(
        "--unified-exit-v2-economics-readiness-json", type=Path, required=True
    )
    parser.add_argument(
        "--train-unified-exit-v2-cost-authority-json", type=Path, required=True
    )
    parser.add_argument(
        "--val-unified-exit-v2-cost-authority-json", type=Path, required=True
    )
    parser.add_argument("--unified-exit-v2-preflight-only", action="store_true")
    ...
    run_train(
        ...
        unified_exit_v2_root_manifest_path=(
            args.unified_exit_lifecycle_v2_root_manifest_json
        ),
        train_unified_exit_v2_compact_path=(
        ...
        unified_exit_v2_economics_readiness_path=(
            args.unified_exit_v2_economics_readiness_json
        ),
        train_unified_exit_v2_cost_authority_path=(
            args.train_unified_exit_v2_cost_authority_json
        ),
        val_unified_exit_v2_cost_authority_path=(
            args.val_unified_exit_v2_cost_authority_json
        ),
        unified_exit_v2_preflight_only=bool(args.unified_exit_v2_preflight_only),
    )
