...
LEGACY: Moved to gx1/inference/_legacy_disabled/ - not used in GX1 runtime/replay.
"""
...
def _load_model_worker_impl(config: ModelLoadConfig) -> ModelLoadResult:
    ...
    try:
        ...
        if config.model_variant == "v10_ctx":
            ...
            # Smoke-only: hard-fail if bundle is not CTX6CAT6 (prevents misconfigured bundle from "passing" smoke)
            ...
            expected_cat = meta.get("expected_ctx_cat_dim")
            expected_cont = meta.get("expected_ctx_cont_dim")
            tag = meta.get("ctx_contract_tag") or meta.get("tag")
            if expected_cat != 6 or expected_cont != 6:
                raise RuntimeError(
                    "[CTX_CONTRACT_SPLIT_BRAIN] model_loader_worker (smoke-only): bundle expected_ctx != 6/6. "
                    f"expected_ctx_cat_dim={expected_cat}, expected_ctx_cont_dim={expected_cont}. "
                    f"Canonical CTX6CAT6 required."
                )
            if tag is not None and tag != "CTX6CAT6":
                raise RuntimeError(
                    "[CTX_CONTRACT_SPLIT_BRAIN] model_loader_worker (smoke-only): bundle ctx tag != CTX6CAT6. "
                    f"tag={tag!r}. Canonical required."
                )
        ...
        if config.model_variant == "v10_ctx":
            ...
            # SYNTHETIC TEST INPUT ONLY (smoke test): zeros(6,6) for ctx_cat/ctx_cont.
            # NOT runtime fallback. Runtime must always supply real CTX6CAT6 from bridge; this worker
            # only verifies the model forward runs. Bundle was already validated as CTX6CAT6 above.
            batch_size = 1
