...
# One owner for the UTC trading-day/session-cycle boundary.  The source
# contract requests OANDA candles with ``alignmentTimezone=UTC``; observed
# weekend or DST gaps are source absence and must never move this analytical
# clock.  Consumers derive labels/ids from this named boundary instead of
# restating 22:00 or using calendar-midnight helpers.
TRADING_SESSION_CLOCK = "UTC"
TRADING_SESSION_BOUNDARY_NAME = "ASIA"
TRADING_SESSION_BOUNDARY_UTC_HOUR = int(
    SESSION_BOUNDARIES[TRADING_SESSION_BOUNDARY_NAME][0]
)
TRADING_SESSION_BOUNDARY_OFFSET = pd.Timedelta(
    hours=TRADING_SESSION_BOUNDARY_UTC_HOUR
)
TRADING_SESSION_DURATION = pd.Timedelta(days=1)
TRADING_SESSION_CLOCK_SCHEMA_VERSION = "xau_trading_session_clock_utc_v1"
...
def _require_explicit_utc_index(
    timestamps: Union[pd.Series, pd.DatetimeIndex, np.ndarray],
    *,
    context: str,
) -> pd.DatetimeIndex:
    """Return a strict UTC index without localizing an implicit clock."""

    if isinstance(timestamps, pd.Series):
        try:
            index = pd.DatetimeIndex(timestamps.array)
        ...
        try:
            ...
            raise RuntimeError(f"{context}_TIMESTAMP_INVALID") from exc
    if index.tz is None:
        raise RuntimeError(f"{context}_CLOCK_IMPLICIT")
    if len(index) and pd.Timestamp(index[0]).utcoffset() != pd.Timedelta(0):
        raise RuntimeError(f"{context}_CLOCK_NOT_UTC")
    if index.hasnans or index.has_duplicates or not index.is_monotonic_increasing:
        raise RuntimeError(f"{context}_TIMESTAMP_ORDER_INVALID")
    return index.tz_convert(TRADING_SESSION_CLOCK)


def trading_session_id_vectorized(
    timestamps: Union[pd.Series, pd.DatetimeIndex, np.ndarray],
    *,
    context: str,
) -> np.ndarray:
    """Return the unique causal trading-day id for each UTC timestamp.

    The integer id changes exactly at the named ASIA/open boundary and is
    independent of whether source rows are absent over weekends or holidays.
    Integer arithmetic avoids a second calendar-midnight owner.
    """
...
def trading_session_label(
    values: pd.Timestamp | pd.DatetimeIndex,
    *,
    context: str,
) -> pd.Timestamp | pd.DatetimeIndex:
    """Floor UTC timestamp(s) to the canonical trading-session opening."""

    scalar = isinstance(values, pd.Timestamp)
    source = pd.DatetimeIndex([values]) if scalar else values
    session_ids = trading_session_id_vectorized(source, context=context)
    labels_ns = (
        session_ids * int(TRADING_SESSION_DURATION.value)
        + int(TRADING_SESSION_BOUNDARY_OFFSET.value)
    )
    labels = pd.DatetimeIndex(labels_ns, tz=TRADING_SESSION_CLOCK)
