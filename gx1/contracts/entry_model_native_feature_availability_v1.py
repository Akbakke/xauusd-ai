...
def _mandatory_family_counts() -> dict[str, int]:
    """Per-specialist counts over the MANDATORY half of the model input.

    Starvation must be judged on the surface the model actually receives
    (rule 2g), which is mandatory ∪ candidate. A family can hold every one of
    its fields as mandatory and still contribute zero candidates — that family
    is fully present in the model, not starved. Imported lazily so this module
    stays importable from the signal contract's own consumers.
    """
    ...
    counts = {name: 0 for name in MODEL_NATIVE_TRAINING_SPECIALISTS}
    for field in MODEL_NATIVE_MANDATORY_SELECTED_FIELDS:
...
def fit_feature_availability_contract(
    *,
    ...
    mandatory_counts = _mandatory_family_counts()
    starved = [
        name
        for name, row in family_coverage.items()
        if row["candidate"] == 0 and mandatory_counts.get(name, 0) == 0
    ]
    if starved:
        raise RuntimeError(
...
def require_feature_availability_contract(
    ...
    *,
    ...
    expected_mandatory = _mandatory_family_counts()
    if value.get("family_coverage") != expected_family or any(
        row["candidate"] == 0 and expected_mandatory.get(name, 0) == 0
        for name, row in expected_family.items()
    ):
