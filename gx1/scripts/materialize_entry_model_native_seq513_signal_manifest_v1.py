...
def load_and_validate_train_feature_ranking(
    ...
    *,
    ...
    # The two rank-source checks that stood here referenced `reference` and
    # `rank_source_sha256`, both of which the TRAIN-rank retirement removed —
    # a guaranteed NameError that made this producer unrunnable. The canonical
    # path/sha identity they guarded is still proven, by
    # `validate_current_pair_source_cascade_proof` below against
    # `source_cascade.canonical_v2_sha256`.
    mtf_path = Path(
    ...
    source_cascade = validate_current_pair_source_cascade_proof(
        ...
        expected_history_start_utc=raw_source_cascade.get("history_start_utc"),
