"""Multi-head Exit-IQL GPU core for fully observable counterfactual rewards.

Mission
-------
Exit-IQL trains on held-trade bar states where forward bars provide the
observed reward for both actions: HOLD and EXIT_NOW.
...
encode different forward exit-evaluation horizons.
...
# --- Exit action contract ---
ACTION_HOLD_ID = 0
ACTION_EXIT_NOW_ID = 1
N_ACTIONS_EXIT = 2
ACTION_LABELS_EXIT = {
    ACTION_HOLD_ID: "HOLD",
    ACTION_EXIT_NOW_ID: "EXIT_NOW",
}
...
def q_ranking_margin_loss(
    ...
    """Per-(state, K) margin-ranking regularizer for the Exit Q-net.
...
def train_multi_head_exit_iql(
    ...
    *,
    ...
    q_ranking_weight: float = 0.0,    # 0.0 = bit-parity (pure MSE); small (~0.05) in retrain
    ...
    # the exact `.mean()` -> bit-parity for the cemented Exit model.
    M_t = None
    ...
    # re-learning from scratch. Loaded
...
def info(model: MultiHeadExitIQLModel) -> dict[str, Any]:
    return {
