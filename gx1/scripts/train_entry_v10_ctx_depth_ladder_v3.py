...
try:
    import gx1.models.entry_v10.entry_v10_ctx_train_v3 as train_mod
    ...
    raise RuntimeError(
        f"[TRAIN_IMPORT_FAIL] Failed to import entry_v10_ctx_train_v3: {type(e).__name__}: {e}"
    ) from e
...
def train_depth_ladder_variant(
    *,
    ...
    try:
        ...
        # Build args for backend trainer (canonical CLI). The trainer requires
        # exactly one of `--dataset_dir` OR `--dataset_manifest`, not both. Prefer
        # manifest when available (richer metadata), else fall back to dir.
        train_args = [
            "--train",
            "--out_bundle_dir", str(variant_out_dir),
        ]
        if dataset_manifest is None:
            train_args.extend(["--dataset_dir", str(dataset_dir)])
        train_path_log = None
        val_path_log = None
        ...
        sys.argv = ["gx1.models.entry_v10.entry_v10_ctx_train_v3"] + train_args
        ...
        # Baseline-verify accepts both V1 (legacy 21-dim) and V3 (45-dim signal_bridge_v3).
        # _CTX_CONT_DIM defaults to 21 from canonical-truth-config; V3 bundles use 45.
        ALLOWED_BRIDGES = {"XGB_SIGNAL_BRIDGE_V1", "XGB_SIGNAL_BRIDGE_V2", "XGB_SIGNAL_BRIDGE_V3"}
        if meta_json.get("signal_bridge_id") not in ALLOWED_BRIDGES:
            raise RuntimeError(
                f"BASELINE_VERIFY_FAILED: signal_bridge_id mismatch: {meta_json.get('signal_bridge_id')} (allowed: {ALLOWED_BRIDGES})"
            )
        if meta_json.get("ctx_tag") != "CTX6CAT6":
            raise RuntimeError(f"BASELINE_VERIFY_FAILED: ctx_tag mismatch: {meta_json.get('ctx_tag')}")
        ALLOWED_CTX_CONT_DIMS = {_CTX_CONT_DIM, 21, 43, 45}
        if int(meta_json.get("ctx_cont_dim") or -1) not in ALLOWED_CTX_CONT_DIMS:
            raise RuntimeError(
                f"BASELINE_VERIFY_FAILED: ctx_cont_dim mismatch: {meta_json.get('ctx_cont_dim')} expected one of {ALLOWED_CTX_CONT_DIMS}"
            )
