...
SSoT / ONE UNIVERSE:
- ctx contract: CTX6CAT6 base (ctx_cat=6 fixed) + 43-feature ctx_cont extension (signal_bridge_v2)
...
- seq: ndarray shaped [seq_len=96, 30]  (signal_bridge_v2 sequence)
- snap: ndarray shaped [30]             (signal_bridge_v2 snapshot)
...
# V2: switch from signal_bridge_v1 (7 fields) to signal_bridge_v2 (30 per-bar fields).
...
from gx1.contracts.signal_bridge_v2 import (
    ...
    ORDERED_BRIDGE_FIELDS_V2 as XGB_BRIDGE_FIELDS,
    ...
    DEFAULT_SEQ_LEN_V2,
    SIGNAL_BRIDGE_ID_V2,
    CONTRACT_SHA256_V2 as SIGNAL_CONTRACT_SHA256,
)
...
def _compute_bad_path_first_n(
    *,
    ...
    Args adverse_threshold_bps + favorable_threshold_bps are kept for backward
    compatibility (recorded in output) but unused by the new logic. The
    horizon_bars now defines the evaluation point (last bar in window).
    """
    if horizon_bars < 1:
    ...
    entry_ask = ask[:-horizon_bars]
    entry_bid = bid[:-horizon_bars]
    # PnL at horizon (close-equivalent at horizon_bars later, in bps).
    # LONG: enter at ask, exit at bid. PnL_long_bps = (exit_bid - entry_ask) / entry_ask * 1e4
    # SHORT: enter at bid, exit at ask. PnL_short_bps = (entry_bid - exit_ask) / entry_bid * 1e4
    ...
    return pd.DataFrame(
        {
            ...
            # New columns for diagnostics / future use
...
def build_dataset_canonical(
    *,
    ...
    # 3) Enforce XGB feature contract order (V2: canonical_v2 87-feature contract for new XGB v3 bundle).
    contract_path = project_root / "gx1" / "xgb" / "contracts" / "xgb_input_features_canonical_v2_v1.json"
    ...
    sanitizer_cfg = project_root / "gx1" / "xgb" / "contracts" / "xgb_input_sanitizer_canonical_v2_v1.json"
    ...
    cv2_needed = list(set(
        ...
        + list(ORDERED_CTX_CONT_NAMES_V2)
    ))
    ...
    missing_ctx = [f for f in ORDERED_CTX_CONT_NAMES_V2 if f not in merged3.columns]
    if missing_ctx:
    ...
    # V2: sig_mat is now (N, 30) per signal_bridge_v2 (XGB-bridge 7 + price-state 23).
    ...
    ctx_cont_names = list(ORDERED_CTX_CONT_NAMES_V2)
...
def main() -> None:
    ...
    parser.add_argument("--seq_len", type=int, default=DEFAULT_SEQ_LEN_V2, help=f"Sequence length for seq feature (default: {DEFAULT_SEQ_LEN_V2}).")
    parser.add_argument(
