...
ACTION = "RUN_R6_RETRAIN_FROM_TAIL_REPAIRED_R5_2_PACKAGE_EXPLICIT_GATE_V1"
LAYER_NAME = ACTION
INPUT_PACKAGE_ROOT = (
...
SELECTED_TAIL_REPAIR_VARIANT = "TAIL_TARGET_WEIGHT_REPAIR_CONSERVATIVE"
SELECTED_TAIL_REPAIR_CANDIDATE_ID = "TAIL_TARGET_WEIGHT_REPAIR_CONSERVATIVE::RECALL"
...
REQUIRED_PACKAGE_FILES = [
    ...
    "tail_repaired_r5_2_candidate_package_integrity_report_v1.json",
    "tail_repaired_r5_2_candidate_package_r6_input_readiness_precheck_v1.json",
    "tail_repaired_r5_2_candidate_oof_scores_v1.csv",
    "tail_repaired_r5_2_candidate_oof_score_provenance_v1.csv",
    "tail_repaired_r5_2_candidate_oof_fold_assignment_v1.csv",
    "tail_repaired_r5_2_candidate_train_validation_membership_v1.csv",
    ...
    "tail_repaired_r5_2_candidate_safety_report_v1.json",
    "tail_repaired_r5_2_candidate_metric_denominator_report_v1.json",
    "tail_repaired_r5_2_candidate_fixed_control_comparison_v1.json",
    "tail_repaired_r5_2_candidate_threshold_selection_report_v1.json",
    "tail_repaired_r5_2_candidate_tail_registry_v1.csv",
    "tail_repaired_r5_2_candidate_tail_registry_v1.json",
    "tail_repaired_r5_2_candidate_tail_gap_decomposition_v1.csv",
    "tail_repaired_r5_2_candidate_tail_gap_decomposition_v1.json",
    "tail_repaired_r5_2_candidate_tail_target_repair_design_v1.csv",
    "tail_repaired_r5_2_candidate_tail_target_repair_design_v1.json",
    "tail_repaired_r5_2_candidate_anti_overfit_audit_v1.json",
    "summary_v1.json",
]

FIXED_CONTROLS = [
    {
        "control_v1": "tail_repaired_r5_2_package_pass_through",
        "bad_v1": 140,
        "tail_v1": 94,
        "role_v1": "TAIL_REPAIRED_R5_2_PACKAGE_PASS_THROUGH_CONTROL_STRICT_LOSO_INVALID_LOW_SUPPORT_VISIBLE",
    },
    {
        "control_v1": "previous_r5_2_package",
        "bad_v1": 130,
        "tail_v1": 86,
        "role_v1": "PREVIOUS_R5_2_PACKAGE_CONTROL",
    },
    {
        "control_v1": "previous_r6_pass_through",
        "bad_v1": 130,
        "tail_v1": 86,
        "role_v1": "PREVIOUS_R6_PASS_THROUGH_CONTROL",
    },
...
def validate_candidate_grid(grid: Sequence[dict[str, Any]]) -> bool:
    ...
    if "TAIL_REPAIRED_R5_2_PASS_THROUGH_CONTROL" not in ids:
        raise RuntimeError("R6_CANDIDATE_GRID_MUST_INCLUDE_TAIL_REPAIRED_R5_2_PASS_THROUGH_CONTROL")
    if len(grid) > 8:
...
def validate_input_package(package_root: Path) -> dict[str, Any]:
    ...
    manifest = _read_json(package_root / "tail_repaired_r5_2_candidate_package_manifest_v1.json")
    integrity = _read_json(package_root / "tail_repaired_r5_2_candidate_package_integrity_report_v1.json")
    precheck = _read_json(package_root / "tail_repaired_r5_2_candidate_package_r6_input_readiness_precheck_v1.json")
    ...
    checks = {
        ...
        "selected_candidate_tail_repair_recall_v1": summary.get("selected_candidate_id_v1")
        == SELECTED_TAIL_REPAIR_CANDIDATE_ID,
        "selected_threshold_recall_v1": summary.get("selected_threshold_candidate_v1") == "RECALL",
        "oof_bad_tail_140_94_v1": summary.get("bad_count_v1") == 140 and summary.get("tail_count_v1") == 94,
        "precision_1_0_v1": float(summary.get("precision_v1", -1.0)) == 1.0,
        "precision_denominator_140_v1": summary.get("precision_denominator_v1") == 140,
        ...
        "selected_low_support_groups_10_v1": summary.get("selected_low_support_group_count_v1") == 10,
        "structural_selected_groups_7_v1": summary.get("structural_low_support_selected_group_count_v1") == 7,
        "safety_clean_v1": summary.get("safety_clean_v1") is True,
        "low_support_report_present_v1": (package_root / "tail_repaired_r5_2_candidate_low_support_report_v1.json").exists(),
        "oof_scores_present_v1": (package_root / "tail_repaired_r5_2_candidate_oof_scores_v1.csv").exists(),
        "oof_provenance_present_v1": (package_root / "tail_repaired_r5_2_candidate_oof_score_provenance_v1.csv").exists(),
        "fold_assignment_present_v1": (package_root / "tail_repaired_r5_2_candidate_oof_fold_assignment_v1.csv").exists(),
        "train_validation_membership_present_v1": (
            package_root / "tail_repaired_r5_2_candidate_train_validation_membership_v1.csv"
        ).exists(),
        "score_source_manifest_present_v1": (
            package_root / "tail_repaired_r5_2_candidate_score_source_manifest_v1.json"
        ).exists(),
        "tail_registry_present_v1": (package_root / "tail_repaired_r5_2_candidate_tail_registry_v1.csv").exists(),
        "tail_gap_decomposition_present_v1": (
            package_root / "tail_repaired_r5_2_candidate_tail_gap_decomposition_v1.csv"
        ).exists(),
        "tail_target_repair_design_present_v1": (
            package_root / "tail_repaired_r5_2_candidate_tail_target_repair_design_v1.csv"
        ).exists(),
        "no_dummy_synthetic_fallback_attestation_present_v1": (
            package_root / "tail_repaired_r5_2_candidate_no_fallback_no_dummy_no_synthetic_attestation_v1.json"
        ).exists(),
...
def _source_mapping(output_dir: Path) -> dict[str, Any]:
    return {
        ...
        "r6_feature_input_source_v1": "v2_replay._prepare_inputs plus explicit tail-repaired R5.2 package scores staged into existing R6 score-frame schema",
...
def _no_reimplementation_attestation(mapping: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "reason_v1": "The new code validates and stages the explicit tail-repaired R5.2 package, then calls existing R6 five-head training and policy utilities in grouped OOF mode.",
...
def _candidate_grid() -> list[dict[str, Any]]:
    return [
        {
            "candidate_id_v1": "TAIL_REPAIRED_R5_2_PASS_THROUGH_CONTROL",
            "candidate_type_v1": "PASS_THROUGH",
            "description_v1": "No R6 filtering beyond the packaged tail-repaired R5.2 selected rows.",
        ...
        {
            "candidate_id_v1": "R6_TAIL_FOCUSED_WITH_HARD_VETO",
            "candidate_type_v1": "R6_POLICY_MASK",
            "family_v1": "R6_R5_2_ULTRA_SAFE_TAIL_RISKY_ADDON",
            "bad_threshold_v1": 0.65,
            "runner_threshold_v1": 0.60,
            "tail_threshold_v1": 0.45,
            "risky_threshold_v1": 0.70,
            "blindspot_threshold_v1": 0.70,
            "r5_2_runner_threshold_v1": 0.74,
            "use_r5_2_base_v1": True,
            "hard_asof_runner_guard_v1": True,
...
def _load_and_stage_package_frame(
    ...
    *,
    ...
    package_scores = pd.read_csv(package_root / "tail_repaired_r5_2_candidate_oof_scores_v1.csv")
    if "tail_repair_variant_id_v1" not in package_scores.columns:
        raise RuntimeError("Tail-repaired package scores missing tail_repair_variant_id_v1")
    package_scores = package_scores[
        package_scores["tail_repair_variant_id_v1"].astype(str).eq(SELECTED_TAIL_REPAIR_VARIANT)
    ].copy()
    if len(package_scores) != len(frame):
        raise RuntimeError(
            "Tail-repaired package scores must align to one selected variant row per foundation row; "
            f"got {len(package_scores)} rows for {len(frame)} foundation rows"
        )
    ...
    package_cols = [
        ...
        "tail_repair_variant_best_selected_v1",
        "fold_id_v1",
    ...
    if missing:
        raise RuntimeError(f"Tail-repaired R5.2 package scores missing required columns for R6 staging: {missing}")
    ...
    if staged["r5_2_coverage_bad_score_v1"].isna().any():
        raise RuntimeError("Tail-repaired R5.2 package scores do not align one-to-one with foundation frame")
    ...
    staged["r5_2_selected_candidate__block_v1"] = _bool(staged, "tail_repair_variant_best_selected_v1")
    staged["r5_2_base_membership_contract_id_v1"] = "TAIL_REPAIRED_R5_2_PACKAGE_RECALL_140_94_EXPLICIT_R6_INPUT"
...
def _fold_assignment(package_root: Path, frame: pd.DataFrame) -> pd.DataFrame:
    raw = pd.read_csv(package_root / "tail_repaired_r5_2_candidate_oof_fold_assignment_v1.csv")
    if "tail_repair_variant_id_v1" in raw.columns:
        raw = raw[raw["tail_repair_variant_id_v1"].astype(str).eq(SELECTED_TAIL_REPAIR_VARIANT)].copy()
    ...
    if out["fold_id_v1"].isna().any():
        raise RuntimeError("Tail-repaired R5.2 package fold assignment does not align one-to-one with R6 staging frame")
    out = out.rename(
        columns={
    ...
    out["split_policy_v1"] = "REUSED_TAIL_REPAIRED_R5_2_PACKAGE_GROUPED_OOF_BY_RUN_ID_FOR_R6"
    return out
    ...
    *,
    ...
    for fold_idx, fold_id in enumerate(sorted(fold_ids.unique())):
        ...
        pred, metrics = existing_r6._train_r6_heads(
            ...
            model_tag=f"r6_from_tail_repaired_r5_2_package_oof_{fold_id}",
                ...
                provenance_rows.append(
                    {
                        ...
                        "variant_v1": "R6_FROM_TAIL_REPAIRED_R5_2_PACKAGE_EXPLICIT_GATE",
                        "model_source_identifier_v1": model_source_id,
                        "train_membership_hash_v1": train_hash,
                        "validation_membership_hash_v1": validation_hash,
...
def _evaluate_candidate(scores: pd.DataFrame, selected: pd.Series, candidate_id: str, config: dict[str, Any]) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    payload = {
        ...
        **loso,
        ...
        "tail_repaired_r5_2_package_rows_retained_v1": int((selected & r5_2_base).sum()),
        "tail_repaired_r5_2_package_rows_blocked_v1": int((~selected & r5_2_base).sum()),
        "tail_repaired_r5_2_package_rows_added_v1": int((selected & ~r5_2_base).sum()),
        "outside_base_rows_used_v1": int((selected & ~r5_2_base).sum()) > 0,
        "outside_base_rows_safety_proven_v1": False,
        "tail_rows_retained_v1": int((selected & r5_2_base & tail).sum()),
        "tail_rows_blocked_v1": int((~selected & r5_2_base & tail).sum()),
        "tail_rows_added_v1": int((selected & ~r5_2_base & tail).sum()),
        "tail_control_10_50_positive_selected_v1": int((selected & (_num(scores, R6_TAIL_PROB, 0.0) >= float(config.get("tail_threshold_v1", 1.0)))).sum())
        if "tail_threshold_v1" in config
        else 0,
...
def _best_status(best: dict[str, Any], *, provenance_ok: bool, no_in_sample_ok: bool, no_overlap_ok: bool) -> tuple[str, str]:
    if not provenance_ok or not no_in_sample_ok or not no_overlap_ok:
        return "R6_TAIL_REPAIRED_CANDIDATE_FAILS_PROVENANCE_OR_IN_SAMPLE_GUARD", "REPAIR_R6_OOF_PROVENANCE_OR_SPLIT_V1"
    if not bool(best.get("safety_clean_v1")):
        return "R6_TAIL_REPAIRED_CANDIDATE_FAILS_TRUE_SAFETY", "REPAIR_R6_SAFETY_HEADS_OR_ADD_HARD_VETO_LAYER_V1"
    bad = int(best.get("bad_count_v1") or 0)
    tail = int(best.get("tail_count_v1") or 0)
    if bad >= 140 and tail >= 94 and (bad > 140 or tail > 94):
        return (
            "R6_TAIL_REPAIRED_CANDIDATE_STRONGER_THAN_140_94_BUT_FINAL_PROMOTION_BLOCKED",
            "MATERIALIZE_TAIL_REPAIRED_R6_CANDIDATE_PACKAGE_REQUIRES_EXPLICIT_GATE_V1",
        )
    if bad >= 140 and tail >= 94:
        return (
            "R6_TAIL_REPAIRED_CANDIDATE_PRESERVES_140_94_WITH_STRONGER_HEAD_DIAGNOSTICS",
            "R6_TAIL_HEAD_CALIBRATION_OR_R5_2_TAIL_GAP_EXPANSION_V1",
        )
    return (
        "R6_TAIL_REPAIRED_CANDIDATE_SAFE_BUT_WEAKER_THAN_140_94",
        "R6_HEAD_SIGNAL_AUDIT_OR_KEEP_TAIL_REPAIRED_R5_2_PACKAGE_AS_CONTROL_V1",
    )
...
def _contract(package_root: Path, package_validation: dict[str, Any]) -> dict[str, Any]:
    return {
        "contract": "R6_FROM_TAIL_REPAIRED_R5_2_PACKAGE_CONTRACT_V1",
        "input_tail_repaired_r5_2_package_root_v1": str(package_root),
        ...
        "tail_repaired_r5_2_package_score_provenance_preserved_v1": True,
        ...
        "tail_specific_diagnostics_required_v1": True,
...
def _write_markdown_artifacts(output_dir: Path, payload: dict[str, Any]) -> None:
    _write_report(
        output_dir / "r6_tail_repaired_input_package_validation_v1.md",
        [
            "# R6 Tail-Repaired Input Package Validation V1",
            "",
    ...
    _write_report(
        output_dir / "r6_tail_repaired_existing_source_mapping_v1.md",
        [
    ...
    _write_report(
        output_dir / "r6_from_tail_repaired_r5_2_package_contract_v1.md",
        [
            "# R6 From Tail-Repaired R5.2 Package Contract V1",
            "",
    ...
    _write_report(
        output_dir / "r6_tail_repaired_fixed_controls_v1.md",
        [
            "# R6 Tail-Repaired Fixed Controls V1",
            "",
    ...
    _write_report(
        output_dir / "r6_tail_repaired_candidate_grid_contract_v1.md",
        [
    ...
    _write_report(
        output_dir / "r6_tail_repaired_best_candidate_v1.md",
        [
...
def materialize(
    *,
    ...
    base_mask = _bool(scores, "r5_2_package_selected_v1")
    tail_mask = _bool(scores, "tail_label_v1")
    bad_mask = _bool(scores, "bad_label_v1")
    unsafe_mask = (
    ...
    metrics_by_id = {str(row["candidate_id_v1"]): row for row in candidate_rows}
    tail_contribution_rows: list[dict[str, Any]] = []
    failed_safe_subset_rows: list[dict[str, Any]] = []
    for row in grid:
        candidate_id = str(row["candidate_id_v1"])
        mask = masks[candidate_id].reindex(scores.index).fillna(False).astype(bool)
        extra = mask & ~base_mask
        blocked = base_mask & ~mask
        tail_threshold = float(row.get("tail_threshold_v1", 1.0)) if "tail_threshold_v1" in row else 1.0
        tail_control_support = _num(scores, R6_TAIL_PROB, 0.0) >= tail_threshold
        tail_contribution_rows.append(
            {
                "candidate_id_v1": candidate_id,
                "tail_repaired_package_tail_retained_v1": int((mask & base_mask & tail_mask).sum()),
                "tail_repaired_package_tail_blocked_v1": int((blocked & tail_mask).sum()),
                "tail_rows_added_v1": int((extra & tail_mask).sum()),
                "tail_rows_added_with_tail_control_10_50_support_v1": int((extra & tail_mask & tail_control_support).sum()),
                "tail_control_10_50_threshold_v1": tail_threshold if "tail_threshold_v1" in row else None,
                "tail_control_10_50_selected_rows_v1": int((mask & tail_control_support).sum()),
                "tail_control_10_50_safe_selected_rows_v1": int((mask & tail_control_support & ~unsafe_mask).sum()),
                "tail_control_10_50_unsafe_selected_rows_v1": int((mask & tail_control_support & unsafe_mask).sum()),
                "safety_clean_v1": bool(metrics_by_id[candidate_id]["safety_clean_v1"]),
            }
        )
        metrics = metrics_by_id[candidate_id]
        if not bool(metrics["safety_clean_v1"]) and (
            int(metrics["bad_count_v1"]) > 140 or int(metrics["tail_count_v1"]) > 94
        ):
            safe_extra = extra & ~unsafe_mask
            unsafe_extra = extra & unsafe_mask
            status_hint = "SAFE_SUBSET_WORTH_MINING" if int((safe_extra & (bad_mask | tail_mask)).sum()) else "TRUE_SAFETY_FAILURE_REJECT"
            failed_safe_subset_rows.append(
                {
                    "candidate_id_v1": candidate_id,
                    "raw_bad_v1": int(metrics["bad_count_v1"]),
                    "raw_tail_v1": int(metrics["tail_count_v1"]),
                    "extra_rows_vs_pass_through_v1": int(extra.sum()),
                    "extra_tail_rows_v1": int((extra & tail_mask).sum()),
                    "unsafe_rows_causing_failure_v1": int(unsafe_extra.sum()),
                    "safe_rows_mixed_with_unsafe_rows_v1": int(safe_extra.sum()),
                    "safe_bad_rows_mixed_v1": int((safe_extra & bad_mask).sum()),
                    "safe_tail_rows_mixed_v1": int((safe_extra & tail_mask).sum()),
                    "hard_veto_could_isolate_safe_subset_v1": bool(int(safe_extra.sum()) and int(unsafe_extra.sum())),
                    "safe_subset_already_in_tail_repair_registry_v1": "UNKNOWN_NOT_MUTATED_IN_THIS_TASK",
                    "safe_subset_worth_future_repair_v1": status_hint == "SAFE_SUBSET_WORTH_MINING",
                    "recommended_status_v1": status_hint,
                }
            )
    if not failed_safe_subset_rows:
        failed_safe_subset_rows.append(
            {
                "candidate_id_v1": "NO_FAILED_HIGHER_RAW_EXPANSION_CANDIDATE",
                "raw_bad_v1": 0,
                "raw_tail_v1": 0,
                "extra_rows_vs_pass_through_v1": 0,
                "extra_tail_rows_v1": 0,
                "unsafe_rows_causing_failure_v1": 0,
                "safe_rows_mixed_with_unsafe_rows_v1": 0,
                "safe_bad_rows_mixed_v1": 0,
                "safe_tail_rows_mixed_v1": 0,
                "hard_veto_could_isolate_safe_subset_v1": False,
                "safe_subset_already_in_tail_repair_registry_v1": False,
                "safe_subset_worth_future_repair_v1": False,
                "recommended_status_v1": "TOO_SMALL_TO_PRIORITIZE",
            }
        )
    best_candidate = {
        "layer_name": "R6_TAIL_REPAIRED_BEST_CANDIDATE_V1",
        "status_v1": status,
        ...
        "selection_priority_v1": [
            ...
            "preserve or beat tail-repaired R5.2 package",
        ],
        "fixed_control_comparison_v1": fixed_comparison,
        "final_promotion_allowed_v1": False,
    }
    go_no_go = {
        "layer_name": "R6_FROM_TAIL_REPAIRED_R5_2_PACKAGE_GO_NO_GO_V1",
        "decision_v1": status,
        "next_recommended_action_v1": next_action,
    ...
    anti_overfit_audit = {
        "layer_name": "R6_TAIL_REPAIRED_ANTI_OVERFIT_AUDIT_V1",
        "status_v1": "R6_TAIL_REPAIRED_STABLE_TRACK_PASS"
        if provenance_check["status_v1"] == "PASS"
        and no_in_sample["status_v1"] == "PASS"
        and no_overlap["status_v1"] == "PASS"
        and bool(best["safety_clean_v1"])
        else "R6_TAIL_REPAIRED_OVERFIT_RISK_DETECTED_STOP",
        "no_optuna_v1": True,
        "no_large_sweep_v1": True,
        "deterministic_candidate_grid_only_v1": True,
        "oof_provenance_status_v1": provenance_check["status_v1"],
        "no_in_sample_decisioning_status_v1": no_in_sample["status_v1"],
        "train_validation_overlap_status_v1": no_overlap["status_v1"],
        "no_post_hoc_threshold_mining_v1": True,
        "safety_clean_required_v1": True,
        "selected_candidate_safety_clean_v1": bool(best["safety_clean_v1"]),
        "strict_loso_visible_v1": True,
        "low_support_visible_v1": True,
        "fixed_controls_included_v1": [row["control_v1"] for row in FIXED_CONTROLS],
        "failed_candidates_not_promoted_v1": True,
        "no_new_feature_surface_v1": True,
        "no_dummy_synthetic_fallback_v1": no_fallback_attestation["status_v1"] == "PASS",
        "wednesday_not_optimized_row_for_row_v1": True,
    }

    package_validation_path = output_dir / "r6_tail_repaired_input_package_validation_v1.json"
    _write_json(package_validation_path, package_validation)
    _write_json(output_dir / "r6_tail_repaired_existing_source_mapping_v1.json", source_mapping)
    _write_json(output_dir / "r6_tail_repaired_no_unnecessary_reimplementation_attestation_v1.json", attestation)
    _write_json(output_dir / "r6_from_tail_repaired_r5_2_package_contract_v1.json", contract)
    _write_json(output_dir / "r6_tail_repaired_fixed_controls_v1.json", {"controls_v1": FIXED_CONTROLS})
    _write_json(output_dir / "r6_tail_repaired_candidate_grid_contract_v1.json", {"contract_v1": "SMALL_DETERMINISTIC_GRID_NO_OPTUNA", "candidate_count_v1": len(grid), "pass_through_required_v1": True})
    _write_rows(output_dir / "r6_tail_repaired_candidate_grid_v1.csv", grid)
    _write_json(output_dir / "r6_tail_repaired_candidate_grid_v1.json", {"candidates_v1": grid})
    scores.to_csv(output_dir / "r6_tail_repaired_oof_scores_v1.csv", index=False)
    provenance.to_csv(output_dir / "r6_tail_repaired_oof_score_provenance_v1.csv", index=False)
    fold_assignment.to_csv(output_dir / "r6_tail_repaired_oof_fold_assignment_v1.csv", index=False)
    membership.to_csv(output_dir / "r6_tail_repaired_train_validation_membership_v1.csv", index=False)
    oof["head_metrics"].to_csv(output_dir / "r6_tail_repaired_oof_head_training_metrics_v1.csv", index=False)
    _write_json(output_dir / "r6_tail_repaired_score_source_manifest_v1.json", score_source_manifest)
    _write_json(output_dir / "r6_tail_repaired_feature_label_hash_manifest_v1.json", feature_label_hash_manifest)
    _write_json(output_dir / "r6_tail_repaired_no_in_sample_decisioning_attestation_v1.json", no_in_sample_attestation)
    _write_json(output_dir / "r6_tail_repaired_no_fallback_no_dummy_no_synthetic_attestation_v1.json", no_fallback_attestation)
    _write_json(output_dir / "r6_tail_repaired_anti_overfit_audit_v1.json", anti_overfit_audit)
    _write_report(
        output_dir / "r6_tail_repaired_anti_overfit_audit_v1.md",
        [
            "# R6 Tail-Repaired Anti-Overfit Audit V1",
            "",
            f"Status: `{anti_overfit_audit['status_v1']}`",
            "Deterministic candidate grid only; no Optuna, broad sweep, freeze, promo, or live action was run.",
            "Strict LOSO and low-support remain visible.",
        ],
    )
    _write_json(
        output_dir / "r6_tail_repaired_candidate_eval_summary_v1.json",
        {
    ...
    _write_rows(output_dir / "r6_tail_repaired_candidate_eval_metrics_v1.csv", candidate_rows)
    _write_rows(output_dir / "r6_tail_repaired_candidate_metric_denominator_report_v1.csv", denominator_rows)
    _write_json(output_dir / "r6_tail_repaired_candidate_metric_denominator_report_v1.json", {"rows_v1": denominator_rows})
    _write_rows(output_dir / "r6_tail_repaired_candidate_safety_report_v1.csv", safety_rows)
    _write_json(output_dir / "r6_tail_repaired_candidate_safety_report_v1.json", {"rows_v1": safety_rows, "safety_clean_v1": best["safety_clean_v1"]})
    _write_rows(output_dir / "r6_tail_repaired_candidate_low_support_report_v1.csv", [low_support_report])
    _write_json(output_dir / "r6_tail_repaired_candidate_low_support_report_v1.json", low_support_report)
    _write_rows(output_dir / "r6_tail_repaired_candidate_fixed_control_comparison_v1.csv", fixed_comparison)
    _write_json(output_dir / "r6_tail_repaired_candidate_fixed_control_comparison_v1.json", {"controls_v1": fixed_comparison})
    _write_rows(output_dir / "r6_tail_repaired_candidate_head_contribution_report_v1.csv", head_contribution_rows)
    _write_json(output_dir / "r6_tail_repaired_candidate_head_contribution_report_v1.json", {"heads_v1": head_contribution_rows})
    _write_rows(output_dir / "r6_tail_repaired_candidate_tail_contribution_report_v1.csv", tail_contribution_rows)
    _write_json(output_dir / "r6_tail_repaired_candidate_tail_contribution_report_v1.json", {"rows_v1": tail_contribution_rows})
    _write_rows(output_dir / "r6_tail_repaired_failed_expansion_safe_subset_analysis_v1.csv", failed_safe_subset_rows)
    _write_json(
        output_dir / "r6_tail_repaired_failed_expansion_safe_subset_analysis_v1.json",
        {"rows_v1": failed_safe_subset_rows},
    )
    _write_report(
        output_dir / "r6_tail_repaired_failed_expansion_safe_subset_report_v1.md",
        [
            "# R6 Tail-Repaired Failed Expansion Safe Subset Analysis V1",
            "",
            "Failed expansion rows are audit-only and do not alter the selected candidate.",
            f"Rows reported: `{len(failed_safe_subset_rows)}`",
        ],
    )
    _write_rows(output_dir / "r6_tail_repaired_candidate_loso_group_detail_v1.csv", loso_detail)
    _write_json(output_dir / "r6_tail_repaired_best_candidate_v1.json", best_candidate)
    _write_json(output_dir / "r6_from_tail_repaired_r5_2_package_go_no_go_v1.json", go_no_go)
    _write_json(
        output_dir / "manifest_v1.json",
        {
            ...
            "input_tail_repaired_r5_2_package_unchanged_v1": package_unchanged,
    ...
    summary = {
        ...
        "tail_repaired_r5_2_package_rows_retained_v1": best["tail_repaired_r5_2_package_rows_retained_v1"],
        "tail_repaired_r5_2_package_rows_blocked_v1": best["tail_repaired_r5_2_package_rows_blocked_v1"],
        "tail_repaired_r5_2_package_rows_added_v1": best["tail_repaired_r5_2_package_rows_added_v1"],
        "tail_rows_retained_v1": best["tail_rows_retained_v1"],
        "tail_rows_added_v1": best["tail_rows_added_v1"],
        "tail_rows_blocked_v1": best["tail_rows_blocked_v1"],
        "oof_provenance_status_v1": provenance_check["status_v1"],
    ...
    _write_report(
        ...
        [
            "# Run R6 Retrain From Tail-Repaired R5.2 Package Explicit Gate V1",
            "",
            f"Go/no-go: `{status}`",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--tail-repaired-r5-2-package-root",
        "--r5-2-package-root",
        dest="tail_repaired_r5_2_package_root",
        type=Path,
        default=INPUT_PACKAGE_ROOT,
    )
    ...
    summary = materialize(
        ...
        package_root=args.tail_repaired_r5_2_package_root,
