...
2. Run XGB v4 per session → 7-dim signal_bridge_v2 per bar.
...
# Force v2 contract (script runs entirely outside the runtime, but we still
# import signal_bridge_v2 directly for the field lists).
os.environ.setdefault("GX1_SIGNAL_BRIDGE_VERSION", "2")
...
ACTION = "INFERENCE_BATCH_CANDIDATES_V2"
...
DEFAULT_XGB_BUNDLE = Path(
    "/home/andre2/GX1_DATA/models/models/xgb_universal_multihead_v4__BIDIR_RSI_SMC_PRUNED_20260503T103505Z"
)
DEFAULT_V10_BUNDLE = Path(
    "/home/andre2/GX1_DATA/models/models/entry_v10_ctx/ENTRY_V10_CTX__RETRAIN_2026Q2_BIDIR_SMC_20260503T134214Z"
)
DEFAULT_XGB_FEATURE_CONTRACT = REPO_ROOT / "gx1" / "xgb" / "contracts" / "xgb_input_features_canonical_v2_v1.json"
DEFAULT_XGB_SANITIZER_CONFIG = REPO_ROOT / "gx1" / "xgb" / "contracts" / "xgb_input_sanitizer_canonical_v2_v1.json"
...
def run_xgb_inference(
    ...
    """Return (n_m5, 7) signal_bridge_v2 per-bar XGB outputs."""
...
def build_v10_input_matrices(
    ...
    per_bar = np.zeros((n, SEQ_SIGNAL_DIM_V2), dtype=np.float32)
    per_bar[:, 0:7] = bridge
    missing_pb = [c for c in PER_BAR_PRICE_STATE_FIELDS_V2 if c not in cv2.columns]
    if missing_pb:
        raise RuntimeError(f"[{ACTION}] PER_BAR_PRICE_STATE missing in prebuilt: {missing_pb}")
    for j, fname in enumerate(PER_BAR_PRICE_STATE_FIELDS_V2):
    ...
    ctx_cont = np.zeros((n, CTX_CONT_DIM_V2), dtype=np.float32)
    for j, fname in enumerate(ORDERED_CTX_CONT_NAMES_V2):
    ...
    ctx_cat = np.zeros((n, CTX_CAT_DIM_V2), dtype=np.int64)
    for j, fname in enumerate(ORDERED_CTX_CAT_NAMES_V2):
        if fname in cv2.columns:
    ...
    *, model: EntryV10CtxHybridTransformer,
    ...
    with torch.no_grad():
        ...
        for batch_start in range(0, n_dec, batch_size):
            ...
            B = len(batch_idx)
            seq_x = np.zeros((B, seq_len, SEQ_SIGNAL_DIM_V2), dtype=np.float32)
