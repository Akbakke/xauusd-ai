...
SWEEP_SPACES: tuple[Space, ...] = (
    ...
    Space("ENTRY_FOUNDATION_CANDIDATE_DIRECTION_SLICE_CONFUSION_PAIR_WEIGHT", "choice", choices=(4.0, 6.0, 8.0)),
    ...
    Space("ENTRY_FOUNDATION_CANDIDATE_HIER_PUBLIC_FLAT_CONSISTENCY_WEIGHT", "choice", choices=(4.0, 6.0, 8.0)),
    Space("ENTRY_FOUNDATION_CANDIDATE_HIER_SLICE_PUBLIC_FLAT_CONSISTENCY_WEIGHT", "choice", choices=(4.0, 6.0, 8.0)),
    Space("ENTRY_FOUNDATION_CANDIDATE_HIER_PUBLIC_TRADE_FLAT_MARGIN_WEIGHT", "choice", choices=(6.0, 8.0)),
    Space("ENTRY_FOUNDATION_CANDIDATE_HIER_SLICE_PUBLIC_TRADE_FLAT_MARGIN_WEIGHT", "choice", choices=(6.0, 8.0)),
...
FIXED_ENV: dict[str, str] = {
    ...
    "ENTRY_FOUNDATION_CANDIDATE_DIRECTION_SLICE_CONFUSION_PAIR_MARGIN": "0.02",
    ...
    "ENTRY_FOUNDATION_CANDIDATE_HIER_PUBLIC_FLAT_CONSISTENCY_MIN_LABEL_RATE": "0.10",
    ...
    "ENTRY_FOUNDATION_CANDIDATE_HIER_PUBLIC_TRADE_FLAT_MARGIN": "0.10",
    "ENTRY_FOUNDATION_CANDIDATE_HIER_PUBLIC_TRADE_FLAT_MARGIN_MIN_LABEL_RATE": "0.10",
    "ENTRY_FOUNDATION_CANDIDATE_HIER_SLICE_PUBLIC_TRADE_FLAT_MARGIN": "0.10",
    "ENTRY_FOUNDATION_CANDIDATE_HIER_SLICE_PUBLIC_TRADE_FLAT_MARGIN_MIN_LABEL_RATE": "0.10",
    "ENTRY_FOUNDATION_CANDIDATE_HIER_SLICE_PUBLIC_TRADE_FLAT_MARGIN_MIN_ROWS": "8",
...
def lint_trial_env(env: dict[str, str]) -> list[str]:
    ...
    if hier_public_direction_composition != "margin_maxnorm_confidence":
        failures.append(
            "HIER_PUBLIC_DIRECTION_COMPOSITION must be margin_maxnorm_confidence for strict XAU repair, "
            f"got {hier_public_direction_composition!r}"
        )
    hier_public_direction_detach_side_grad = int(
        float(env.get("ENTRY_FOUNDATION_CANDIDATE_HIER_PUBLIC_DIRECTION_DETACH_SIDE_GRAD", "0"))
    )
    if hier_public_direction_detach_side_grad != 1:
        failures.append(
            "HIER_PUBLIC_DIRECTION_DETACH_SIDE_GRAD must be 1 for strict XAU repair, "
            f"got {hier_public_direction_detach_side_grad}"
        )
    ...
    if hier_public_side_head != 1:
        failures.append(
            "HIER_PUBLIC_SIDE_HEAD must be 1 for strict XAU repair, "
            f"got {hier_public_side_head}"
        )
    hier_public_side_dir_margin_bridge = int(
        float(env.get("ENTRY_FOUNDATION_CANDIDATE_HIER_PUBLIC_SIDE_DIR_MARGIN_BRIDGE", "0"))
    )
    if hier_public_side_dir_margin_bridge != 1:
        failures.append(
            "HIER_PUBLIC_SIDE_DIR_MARGIN_BRIDGE must be 1 for strict XAU repair, "
            f"got {hier_public_side_dir_margin_bridge}"
        )
    hier_public_side_dir_margin_bridge_scale = float(
        env.get("ENTRY_FOUNDATION_CANDIDATE_HIER_PUBLIC_SIDE_DIR_MARGIN_BRIDGE_SCALE", "0")
    )
    if (
        hier_public_side_dir_margin_bridge_scale < 0.25
        or hier_public_side_dir_margin_bridge_scale > 1.00
    ):
        failures.append(
            "HIER_PUBLIC_SIDE_DIR_MARGIN_BRIDGE_SCALE must stay within [0.25, 1.00] for strict XAU repair, "
            f"got {hier_public_side_dir_margin_bridge_scale}"
        )
    hier_public_side_dir_margin_bridge_cap = float(
        env.get("ENTRY_FOUNDATION_CANDIDATE_HIER_PUBLIC_SIDE_DIR_MARGIN_BRIDGE_CAP", "0")
    )
    if (
        hier_public_side_dir_margin_bridge_cap < 0.05
        or hier_public_side_dir_margin_bridge_cap > 0.50
    ):
        failures.append(
    ...
    hier_public_flat_consistency_weight = float(
        env.get("ENTRY_FOUNDATION_CANDIDATE_HIER_PUBLIC_FLAT_CONSISTENCY_WEIGHT", "0")
    )
    hier_public_flat_consistency_min_label_rate = float(
        env.get("ENTRY_FOUNDATION_CANDIDATE_HIER_PUBLIC_FLAT_CONSISTENCY_MIN_LABEL_RATE", "0")
    )
    hier_slice_public_flat_consistency_weight = float(
        env.get("ENTRY_FOUNDATION_CANDIDATE_HIER_SLICE_PUBLIC_FLAT_CONSISTENCY_WEIGHT", "0")
    )
    hier_slice_public_flat_consistency_min_label_rate = float(
        env.get("ENTRY_FOUNDATION_CANDIDATE_HIER_SLICE_PUBLIC_FLAT_CONSISTENCY_MIN_LABEL_RATE", "0")
    )
    hier_slice_public_flat_consistency_min_rows = int(
        float(env.get("ENTRY_FOUNDATION_CANDIDATE_HIER_SLICE_PUBLIC_FLAT_CONSISTENCY_MIN_ROWS", "0"))
    )
    hier_public_trade_flat_margin_weight = float(
        env.get("ENTRY_FOUNDATION_CANDIDATE_HIER_PUBLIC_TRADE_FLAT_MARGIN_WEIGHT", "0")
    )
    hier_public_trade_flat_margin = float(
        env.get("ENTRY_FOUNDATION_CANDIDATE_HIER_PUBLIC_TRADE_FLAT_MARGIN", "0")
    )
    hier_public_trade_flat_margin_min_label_rate = float(
        env.get("ENTRY_FOUNDATION_CANDIDATE_HIER_PUBLIC_TRADE_FLAT_MARGIN_MIN_LABEL_RATE", "0")
    )
    hier_slice_public_trade_flat_margin_weight = float(
        env.get("ENTRY_FOUNDATION_CANDIDATE_HIER_SLICE_PUBLIC_TRADE_FLAT_MARGIN_WEIGHT", "0")
    )
    hier_slice_public_trade_flat_margin = float(
        env.get("ENTRY_FOUNDATION_CANDIDATE_HIER_SLICE_PUBLIC_TRADE_FLAT_MARGIN", "0")
    )
    hier_slice_public_trade_flat_margin_min_label_rate = float(
        env.get("ENTRY_FOUNDATION_CANDIDATE_HIER_SLICE_PUBLIC_TRADE_FLAT_MARGIN_MIN_LABEL_RATE", "0")
    )
    hier_slice_public_trade_flat_margin_min_rows = int(
        float(env.get("ENTRY_FOUNDATION_CANDIDATE_HIER_SLICE_PUBLIC_TRADE_FLAT_MARGIN_MIN_ROWS", "0"))
    )
    ...
    if hier_public_flat_consistency_weight < 4.0:
        failures.append(
            "HIER_PUBLIC_FLAT_CONSISTENCY_WEIGHT must be >= 4.0 for strict XAU repair, "
            f"got {hier_public_flat_consistency_weight}"
        )
    if hier_public_flat_consistency_min_label_rate < 0.10:
        failures.append(
            "HIER_PUBLIC_FLAT_CONSISTENCY_MIN_LABEL_RATE must be >= 0.10 for strict XAU repair, "
            f"got {hier_public_flat_consistency_min_label_rate}"
        )
    if hier_slice_public_flat_consistency_weight < 4.0:
        failures.append(
            "HIER_SLICE_PUBLIC_FLAT_CONSISTENCY_WEIGHT must be >= 4.0 for strict XAU repair, "
            f"got {hier_slice_public_flat_consistency_weight}"
        )
    if hier_slice_public_flat_consistency_min_label_rate < 0.10:
        failures.append(
            "HIER_SLICE_PUBLIC_FLAT_CONSISTENCY_MIN_LABEL_RATE must be >= 0.10 for strict XAU repair, "
            f"got {hier_slice_public_flat_consistency_min_label_rate}"
        )
    if hier_slice_public_flat_consistency_min_rows < 8:
        failures.append(
            "HIER_SLICE_PUBLIC_FLAT_CONSISTENCY_MIN_ROWS must be >= 8 for strict XAU repair, "
            f"got {hier_slice_public_flat_consistency_min_rows}"
        )
    if hier_public_trade_flat_margin_weight < 6.0:
        failures.append(
            "HIER_PUBLIC_TRADE_FLAT_MARGIN_WEIGHT must be >= 6.0 for strict XAU repair, "
            f"got {hier_public_trade_flat_margin_weight}"
        )
    if hier_public_trade_flat_margin < 0.10:
        failures.append(
            "HIER_PUBLIC_TRADE_FLAT_MARGIN must be >= 0.10 for strict XAU repair, "
            f"got {hier_public_trade_flat_margin}"
        )
    if hier_public_trade_flat_margin_min_label_rate < 0.10:
        failures.append(
            "HIER_PUBLIC_TRADE_FLAT_MARGIN_MIN_LABEL_RATE must be >= 0.10 for strict XAU repair, "
            f"got {hier_public_trade_flat_margin_min_label_rate}"
        )
    if hier_slice_public_trade_flat_margin_weight < 6.0:
        failures.append(
            "HIER_SLICE_PUBLIC_TRADE_FLAT_MARGIN_WEIGHT must be >= 6.0 for strict XAU repair, "
            f"got {hier_slice_public_trade_flat_margin_weight}"
        )
    if hier_slice_public_trade_flat_margin < 0.10:
        failures.append(
            "HIER_SLICE_PUBLIC_TRADE_FLAT_MARGIN must be >= 0.10 for strict XAU repair, "
            f"got {hier_slice_public_trade_flat_margin}"
        )
    if hier_slice_public_trade_flat_margin_min_label_rate < 0.10:
        failures.append(
            "HIER_SLICE_PUBLIC_TRADE_FLAT_MARGIN_MIN_LABEL_RATE must be >= 0.10 for strict XAU repair, "
            f"got {hier_slice_public_trade_flat_margin_min_label_rate}"
        )
    if hier_slice_public_trade_flat_margin_min_rows < 8:
        failures.append(
            "HIER_SLICE_PUBLIC_TRADE_FLAT_MARGIN_MIN_ROWS must be >= 8 for strict XAU repair, "
            f"got {hier_slice_public_trade_flat_margin_min_rows}"
        )
    ...
    slice_confusion_pair_weight = float(
        env.get("ENTRY_FOUNDATION_CANDIDATE_DIRECTION_SLICE_CONFUSION_PAIR_WEIGHT", "0")
    )
    slice_confusion_pair_margin = float(
        env.get("ENTRY_FOUNDATION_CANDIDATE_DIRECTION_SLICE_CONFUSION_PAIR_MARGIN", "0")
    )
    if slice_confusion_pair_weight < 4.0:
        failures.append(
            "DIRECTION_SLICE_CONFUSION_PAIR_WEIGHT must be >= 4.0 for strict XAU repair, "
            f"got {slice_confusion_pair_weight}"
        )
    if slice_confusion_pair_margin < 0.02:
        failures.append(
            "DIRECTION_SLICE_CONFUSION_PAIR_MARGIN must be >= 0.02 for strict XAU repair, "
            f"got {slice_confusion_pair_margin}"
        )
