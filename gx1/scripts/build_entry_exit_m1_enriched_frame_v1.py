...
M1_DURATION = pd.Timedelta(seconds=EXIT_DECISION_BAR_SECONDS)
...
def _load_native_m1(root: Path) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    for raw_path in identity["part_paths"]:
        ...
        if tuple(frame.columns) != RAW_COLUMNS:
            raise RuntimeError(
                "M1_ENRICHED_SOURCE_SCHEMA_INVALID: "
                f"{raw_path} expected={RAW_COLUMNS} observed={tuple(frame.columns)}"
            )
        frames.append(frame)
    frame = pd.concat(frames, ignore_index=True)
    if frame.empty:
        raise RuntimeError("M1_ENRICHED_SOURCE_EMPTY")
    frame["time"] = pd.to_datetime(frame["time"], utc=True, errors="coerce")
    if frame["time"].isna().any():
        raise RuntimeError("M1_ENRICHED_SOURCE_TIME_INVALID")
    ...
    if times.has_duplicates or not times.is_monotonic_increasing:
        raise RuntimeError("M1_ENRICHED_SOURCE_TIME_ORDER_INVALID")
    if np.any(times.asi8 % int(M1_DURATION.value) != 0):
        raise RuntimeError("M1_ENRICHED_SOURCE_OFF_M1_GRID")
    ...
    if not np.isfinite(values).all():
        raise RuntimeError("M1_ENRICHED_SOURCE_NONFINITE")
    ...
    if (
        ...
        raise RuntimeError("M1_ENRICHED_SOURCE_OHLCV_GEOMETRY_INVALID")
...
def _checkpoint_key(
    *,
    ...
    return _canonical_sha256(
        {
            "schema_version": "gx1_m1_enriched_frame_checkpoint_key_v1",
...
def build_m1_enriched_frame(
    *,
    ...
    if (
        ...
        raise RuntimeError("M1_ENRICHED_WORKERS_INVALID")
    if (
        isinstance(checkpoint_chunk_rows, bool)
        or not isinstance(checkpoint_chunk_rows, int)
        or checkpoint_chunk_rows <= 0
    ):
        raise RuntimeError("M1_ENRICHED_CHECKPOINT_CHUNK_ROWS_INVALID")
    if (
        ...
        raise RuntimeError("M1_ENRICHED_RANK_REFERENCE_SHA256_INVALID")
    ...
    if output.exists() or output.is_symlink() or manifest.exists() or manifest.is_symlink():
        raise RuntimeError("M1_ENRICHED_OUTPUT_ALREADY_EXISTS")
    output.parent.mkdir(parents=True, exist_ok=True)
    manifest.parent.mkdir(parents=True, exist_ok=True)
    checkpoint = Path(checkpoint_dir).expanduser().resolve()
    if checkpoint.is_symlink():
        raise RuntimeError("M1_ENRICHED_CHECKPOINT_SYMLINK")
    ...
    if observed_rank_sha256 != rank_reference_sha256:
        raise RuntimeError(
            "M1_ENRICHED_RANK_REFERENCE_HASH_MISMATCH: "
    ...
    enriched = attach_group_a_dip_struct_ctx_columns_parallel(
        canonical,
        multi_tf=multi_tf,
        journal_label="m1_enriched_frame",
        workers=workers,
    ...
    if missing:
        raise RuntimeError(f"M1_ENRICHED_OUTPUT_FIELDS_MISSING: {missing}")
    ...
    for name in OUTPUT_COLUMNS[1:]:
        ...
        if not np.isfinite(values).all():
            raise RuntimeError(f"M1_ENRICHED_OUTPUT_NONFINITE: {name}")
    cats = output_frame[list(MODEL_NATIVE_CTX_CAT_FIELDS)].to_numpy(dtype=np.float64)
    if not np.equal(cats, np.rint(cats)).all():
        raise RuntimeError("M1_ENRICHED_OUTPUT_CTX_CAT_NONINTEGER")
    output_frame.to_parquet(output, index=False)

    result = {
        "schema_version": "gx1_m1_enriched_causal_frame_v1",
        "decision": "PASS",
        ...
        "causal_contract": {
            ...
            "decision_uses_closed_m1_bar": True,
...
def main() -> None:
    ...
    result = build_m1_enriched_frame(
