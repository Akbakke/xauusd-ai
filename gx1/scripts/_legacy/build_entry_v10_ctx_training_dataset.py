...
def _run_prebuilt_split_only(truth_file: Path, train_output: Path, val_output: Path) -> None:
    """
    Regenerate train/val parquets from canonical prebuilt (STRICT ctx 6/6).
    No feature build; load prebuilt from truth, validate, time-split, sort, write.
    """
    with open(truth_file, "r", encoding="utf-8") as f:
        truth = json.load(f)
    prebuilt_path_str = truth.get("canonical_prebuilt_parquet")
    if not prebuilt_path_str:
        raise RuntimeError(
            "STRICT_PREBUILT_SPLIT: canonical_prebuilt_parquet missing in truth file"
        )
    prebuilt_path = Path(prebuilt_path_str).expanduser().resolve()
    if not prebuilt_path.exists():
        raise FileNotFoundError(
            f"STRICT_PREBUILT_SPLIT: canonical prebuilt not found: {prebuilt_path}"
        )

    log.info("[PREBUILT_SPLIT] Loading prebuilt: %s", prebuilt_path)
    ...
    if "ts" in df.columns:
        ...
        raise RuntimeError(
            "STRICT_PREBUILT_SPLIT: prebuilt must have 'ts' or 'time' column or DatetimeIndex"
        )
    df[ts_col] = pd.to_datetime(df[ts_col], utc=True, errors="coerce")
    if df[ts_col].isna().any():
        n_na = int(df[ts_col].isna().sum())
        raise RuntimeError(
            f"STRICT_PREBUILT_SPLIT: {ts_col} has {n_na} NaT rows (hard-fail)"
        )

    ctx_cont_names = list(ORDERED_CTX_CONT_NAMES_EXTENDED[:6])
    ctx_cat_names = list(ORDERED_CTX_CAT_NAMES_EXTENDED[:6])
    missing_cont = [c for c in ctx_cont_names if c not in df.columns]
    missing_cat = [c for c in ctx_cat_names if c not in df.columns]
    if missing_cont or missing_cat:
        raise RuntimeError(
            f"STRICT_PREBUILT_SPLIT: missing contract columns: "
            f"cont={missing_cont} cat={missing_cat}"
        )

    for col in ["vol_regime_id", "atr_bucket"]:
        if col not in df.columns:
            raise RuntimeError(f"STRICT_PREBUILT_SPLIT: missing column {col}")
        lo, hi = df[col].min(), df[col].max()
        if lo < 0 or hi > 3:
            raise RuntimeError(
                f"STRICT_PREBUILT_SPLIT: {col} must be in [0,3], got min={lo} max={hi}"
            )
    if "spread_bucket" not in df.columns:
        raise RuntimeError("STRICT_PREBUILT_SPLIT: missing column spread_bucket")
    lo, hi = df["spread_bucket"].min(), df["spread_bucket"].max()
    if lo < 0 or hi > 2:
        raise RuntimeError(
            f"STRICT_PREBUILT_SPLIT: spread_bucket must be in [0,2], got min={lo} max={hi}"
        )

    train_start = pd.Timestamp("2025-01-01", tz="UTC")
    train_end = pd.Timestamp("2025-09-30 23:59:59.999999", tz="UTC")
    val_start = pd.Timestamp("2025-10-01", tz="UTC")
    val_end = pd.Timestamp("2025-12-31 23:59:59.999999", tz="UTC")

    df_train = df[(df[ts_col] >= train_start) & (df[ts_col] <= train_end)].copy()
    df_val = df[(df[ts_col] >= val_start) & (df[ts_col] <= val_end)].copy()
    df_train = df_train.sort_values(ts_col).reset_index(drop=True)
    df_val = df_val.sort_values(ts_col).reset_index(drop=True)

    # EntryV10CtxDataset requires "time" column; prebuilt may have index as time → we set ts_col "ts"
    if "time" not in df_train.columns and ts_col in df_train.columns:
        df_train["time"] = df_train[ts_col]
        df_val["time"] = df_val[ts_col]

    train_output = Path(train_output).resolve()
    val_output = Path(val_output).resolve()
    ...
    log.info("Saved train: %s (%d rows)", train_output, len(df_train))
    log.info("Saved val:   %s (%d rows)", val_output, len(df_val))

    # Read-back assert and DONE log
    t = pd.read_parquet(train_output)
    v = pd.read_parquet(val_output)
    for name, frame in [("train", t), ("val", v)]:
        vol_lo, vol_hi = int(frame["vol_regime_id"].min()), int(frame["vol_regime_id"].max())
        atr_lo, atr_hi = int(frame["atr_bucket"].min()), int(frame["atr_bucket"].max())
        spr_lo, spr_hi = int(frame["spread_bucket"].min()), int(frame["spread_bucket"].max())
        if vol_lo < 0 or vol_hi > 3 or atr_lo < 0 or atr_hi > 3 or spr_lo < 0 or spr_hi > 2:
            raise RuntimeError(
                f"STRICT_PREBUILT_SPLIT: read-back {name} bucket out of range: "
                f"vol=[{vol_lo},{vol_hi}] atr=[{atr_lo},{atr_hi}] spread=[{spr_lo},{spr_hi}]"
            )
    train_vol = (int(t["vol_regime_id"].min()), int(t["vol_regime_id"].max()))
    train_atr = (int(t["atr_bucket"].min()), int(t["atr_bucket"].max()))
    train_spr = (int(t["spread_bucket"].min()), int(t["spread_bucket"].max()))
    val_vol = (int(v["vol_regime_id"].min()), int(v["vol_regime_id"].max()))
    val_atr = (int(v["atr_bucket"].min()), int(v["atr_bucket"].max()))
    val_spr = (int(v["spread_bucket"].min()), int(v["spread_bucket"].max()))
    log.info(
        "DONE prebuilt split | train=%s (%s rows) vol_regime_id=%s atr_bucket=%s spread_bucket=%s | val=%s (%s rows) vol_regime_id=%s atr_bucket=%s spread_bucket=%s",
        train_output, len(t), train_vol, train_atr, train_spr,
        val_output, len(v), val_vol, val_atr, val_spr,
    )


def main():
    parser = argparse.ArgumentParser(description="Build ENTRY_V10_CTX training dataset")
    parser.add_argument("--data", type=str, default=None, help="Input data file (parquet)")
    parser.add_argument("--output", type=str, default=None, help="Output dataset file (parquet)")
    parser.add_argument("--truth-file", type=str, default=None, help="Truth JSON; use canonical_prebuilt_parquet as input (with --train-output and --val-output)")
    parser.add_argument("--train-output", type=str, default=None, help="Exact path for train parquet (prebuilt-split mode)")
    parser.add_argument("--val-output", type=str, default=None, help="Exact path for val parquet (prebuilt-split mode)")
    parser.add_argument("--policy_config", type=str, default=None, help="Policy config YAML/JSON")
    parser.add_argument("--feature_meta_path", type=str, default=None, help="Feature metadata JSON")
    ...
    # Prebuilt-split-only mode: from truth prebuilt -> exact train/val paths (STRICT 6/6)
    if args.train_output and args.val_output:
        truth_path = Path(
            args.truth_file
            or os.environ.get(
                "GX1_CANONICAL_TRUTH_FILE",
                "/home/andre2/src/GX1_ENGINE/gx1/configs/canonical_truth_signal_only.json",
            )
        ).expanduser().resolve()
        if not truth_path.exists():
            raise FileNotFoundError(f"Truth file not found: {truth_path}")
        _run_prebuilt_split_only(
            truth_path,
            Path(args.train_output).resolve(),
            Path(args.val_output).resolve(),
        )
        return

    if not args.data or not args.output:
        parser.error("--data and --output are required when not using --train-output and --val-output")
    if not args.policy_config or not args.feature_meta_path:
        parser.error("--policy_config and --feature_meta_path are required for full build")

    # Lazy imports for full build only (avoid runtime_v9 etc. when using prebuilt-split mode)
