...
# The runtime-head schema version has exactly one owner,
# `entry_model_native_runtime_evidence_v1`. It used to be duplicated here and
# guarded by an equality check; the two were then bumped independently (v10
# here, v12 there), so the guard rejected every runtime_authoritative
# prediction event. The duplicate is removed rather than re-synced — a second
# copy of a version literal can only ever drift again (rule 13).
...
def validate_runtime_head_parquet_semantics(
    ...
    *,
    ...
    if runtime_schema != [MODEL_NATIVE_RUNTIME_HEAD_EVIDENCE_SCHEMA_VERSION]:
        raise RuntimeError(
