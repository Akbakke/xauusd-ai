#!/usr/bin/env python3
"""
A/B evaluation of Q4 A_TREND overlay impact (chunk-JSON only, no merge dependency).
...
- Q4 × A_TREND total
- Q4 × A_TREND × session (EU/OVERLAP/US, min 200 trades)
- Effective risk reduction proxy (p90_loss comparison)

Output:
reports/SNIPER_Q4_ATREND_OVERLAY_AB__YYYYMMDD_HHMMSS.md
"""
...
def extract_trade_row(trade: Dict[str, Any], run_label: str) -> Optional[Dict[str, Any]]:
    ...
    row = {
        ...
        "quarter": quarter,
        "overlay_applied": atrend_overlay.get("overlay_applied", False) if atrend_overlay else False,
        "overlay_multiplier": atrend_overlay.get("multiplier") if atrend_overlay else None,
        "size_before_units": atrend_overlay.get("size_before_units") if atrend_overlay else None,
        "size_after_units": atrend_overlay.get("size_after_units") if atrend_overlay else None,
    }
...
def generate_ab_report(
    ...
    # Q4 × A_TREND metrics
    df_a_atrend = df_a[df_a["regime_class"] == "A_TREND"]
    df_b_atrend = df_b[df_b["regime_class"] == "A_TREND"]
    metrics_a_atrend = compute_metrics(df_a_atrend.to_dict("records"))
    metrics_b_atrend = compute_metrics(df_b_atrend.to_dict("records"))
    
    # Q4 × A_TREND × Session metrics (min 200 trades)
    session_metrics: Dict[str, Dict[str, Dict[str, Any]]] = {}
    for session in ["EU", "OVERLAP", "US"]:
        df_a_sess = df_a[(df_a["regime_class"] == "A_TREND") & (df_a["session"] == session)]
        df_b_sess = df_b[(df_b["regime_class"] == "A_TREND") & (df_b["session"] == session)]
        if len(df_a_sess) >= 200 or len(df_b_sess) >= 200:
            session_metrics[session] = {
                "a": compute_metrics(df_a_sess.to_dict("records")),
                "b": compute_metrics(df_b_sess.to_dict("records")),
            }
    ...
    delta_atrend = {
        "ev": metrics_b_atrend["ev_per_trade"] - metrics_a_atrend["ev_per_trade"],
        "winrate": metrics_b_atrend["winrate"] - metrics_a_atrend["winrate"],
        "payoff": metrics_b_atrend["payoff"] - metrics_a_atrend["payoff"],
        "p90_loss": metrics_b_atrend["p90_loss"] - metrics_a_atrend["p90_loss"],
    }
    ...
    report_lines = [
        ...
        "| Metric | Run A (OFF) | Run B (ON) | Delta |",
        "|--------|-------------|------------|-------|",
        f"| Trades | {metrics_a_atrend['n_trades']} | {metrics_b_atrend['n_trades']} | {metrics_b_atrend['n_trades'] - metrics_a_atrend['n_trades']:+d} |",
        f"| EV/Trade | {metrics_a_atrend['ev_per_trade']:.2f} bps | {metrics_b_atrend['ev_per_trade']:.2f} bps | {delta_atrend['ev']:+.2f} bps |",
        f"| Winrate | {metrics_a_atrend['winrate']:.1%} | {metrics_b_atrend['winrate']:.1%} | {delta_atrend['winrate']:+.1%} |",
        f"| Payoff | {metrics_a_atrend['payoff']:.2f} | {metrics_b_atrend['payoff']:.2f} | {delta_atrend['payoff']:+.2f} |",
        f"| P90 Loss | {metrics_a_atrend['p90_loss']:.2f} bps | {metrics_b_atrend['p90_loss']:.2f} bps | {delta_atrend['p90_loss']:+.2f} bps |",
        "",
    ]
    ...
    if session_metrics:
        ...
        for session in ["EU", "OVERLAP", "US"]:
            if session in session_metrics:
                m_a = session_metrics[session]["a"]
                m_b = session_metrics[session]["b"]
                delta_sess = {
                    "ev": m_b["ev_per_trade"] - m_a["ev_per_trade"],
                    "p90_loss": m_b["p90_loss"] - m_a["p90_loss"],
                }
                report_lines.extend([
                    f"#### {session}",
                    "",
                    "| Metric | Run A (OFF) | Run B (ON) | Delta |",
                    "|--------|-------------|------------|-------|",
                    f"| Trades | {m_a['n_trades']} | {m_b['n_trades']} | {m_b['n_trades'] - m_a['n_trades']:+d} |",
                    f"| EV/Trade | {m_a['ev_per_trade']:.2f} bps | {m_b['ev_per_trade']:.2f} bps | {delta_sess['ev']:+.2f} bps |",
                    f"| P90 Loss | {m_a['p90_loss']:.2f} bps | {m_b['p90_loss']:.2f} bps | {delta_sess['p90_loss']:+.2f} bps |",
                    "",
                ])
    ...
    # EV change
    if delta_atrend["ev"] > 0:
        conclusions.append(f"✅ **EV/Trade increased** in A_TREND: {delta_atrend['ev']:+.2f} bps")
    elif delta_atrend["ev"] < 0:
        conclusions.append(f"⚠️  **EV/Trade decreased** in A_TREND: {delta_atrend['ev']:+.2f} bps")
    else:
        conclusions.append("➡️  **EV/Trade unchanged** in A_TREND")
    
    # P90 loss change (negative delta is better)
    if delta_atrend["p90_loss"] < 0:
        conclusions.append(f"✅ **P90 Loss reduced** in A_TREND: {delta_atrend['p90_loss']:+.2f} bps (target achieved)")
    elif delta_atrend["p90_loss"] > 0:
        conclusions.append(f"⚠️  **P90 Loss increased** in A_TREND: {delta_atrend['p90_loss']:+.2f} bps")
    else:
        conclusions.append("➡️  **P90 Loss unchanged** in A_TREND")
    
    # Total EV change
    if delta_total["ev"] > 0:
        conclusions.append(f"✅ **Total EV/Trade increased**: {delta_total['ev']:+.2f} bps")
    elif delta_total["ev"] < 0:
        conclusions.append(f"⚠️  **Total EV/Trade decreased**: {delta_total['ev']:+.2f} bps")
...
def main():
    parser = argparse.ArgumentParser(
        description="A/B evaluation of Q4 A_TREND overlay impact"
    )
    parser.add_argument(
        "--run-a",
        type=str,
    ...
    report_file = REPORTS_DIR / f"SNIPER_Q4_ATREND_OVERLAY_AB__{timestamp}.md"
