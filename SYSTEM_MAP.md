...
- **Entry Transformer full feature audit (2026-06-26):** run `python -m gx1.audit.entry_transformer_feature_audit` before V10 retrain. It covers all 294 named input surfaces (41 seq/snap + 123 ctx_cont + 5 ctx_cat + 5×25 MTF) and writes CSV/JSON under `/home/andre2/GX1_DATA/reports/entry_transformer_feature_audit_20260626`. Current findings: old 6yr train parquet has corrupt `body_pct` snap/seq_last max `1.196e10` (fixed in canonical builder; **dataset rebuild required**), `spread_bps`/`spread_bucket` are structural-dead in OHLC prebuilt, `smc_choch` is a too-sparse flip-bar bit (~0.1% nonzero), D1 regime/structure cols can be constant in OOT windows, MTF cache is active/dimension-correct. Bundle metadata ctx-name length mismatch (21 vs 123) was fixed in writer and existing active/candidate metadata (backups `.bak_ctx21_20260626`).
