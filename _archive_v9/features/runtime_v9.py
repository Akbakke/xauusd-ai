...
def build_v9_live_base_features(df_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    norm_to_src = defaultdict(list)
    col_mapping = {}
    for col in df.columns:
        col_lower = col.lower()
        if col_lower in required_cols:
            norm_to_src[col_lower].append(col)
            col_mapping[col] = col_lower
    
    # Detect collisions before rename (multiple source columns → same normalized name)
    collisions = {norm: srcs for norm, srcs in norm_to_src.items() if len(srcs) > 1}
    if collisions:
        raise V9RuntimeFeatureError(
            "Duplicate columns after normalization (case-insensitive collision). "
            f"Collisions: {collisions}. "
            "Fix the upstream dataframe columns so each required column maps uniquely."
        )
    ...
    # Extra safety guard: check for any duplicate columns after rename
    if df.columns.duplicated().any():
        dupes = df.columns[df.columns.duplicated()].tolist()
        raise V9RuntimeFeatureError(
            f"DataFrame has duplicate columns after rename: {dupes}. "
            "This should not happen if collision detection worked correctly."
        )
    ...
    # 0) Sanitize OHLCV dtypes (FIX: ensure float32/float64 to avoid pandas object dtype path)
    import os
    import numpy as np
    ...
    for col in ohlcv_cols:
        if col in df.columns:
            if df[col].dtype not in [np.float32, np.float64]:
                # Hard fail in replay mode if dtype is wrong (indicates data corruption)
                if is_replay:
                    raise ValueError(
                        f"OHLCV dtype mismatch (replay): column '{col}' has dtype {df[col].dtype}, "
                        f"expected float32/float64. This may cause pandas timeout. "
                        f"First 5 values: {df[col].head().tolist()}"
                    )
                # Live mode: convert with warning
                log.warning(
                    "[ENTRY_V9] Converting column '%s' from %s to float32 (expected float32/float64)",
                    col,
                    df[col].dtype
                )
    ...
    # 1.5) Inject incremental quantile features if GX1_REPLAY_INCREMENTAL_FEATURES=1
    # (build_basic_v1 skips batch quantile computation when this flag is set)
    ...
    if use_incremental:
        ...
        if state is not None and state.r1_quantiles_state is not None:
            ...
            if len(df) > 0 and 'close' in df.columns:
                ...
                # Inject quantile values (set for all rows, but only last row matters for snapshot)
                df["_v1_r1_q10_48"] = q10_val
                df["_v1_r1_q90_48"] = q90_val
                log.debug(
                    "[ENTRY_V9] Runtime: injected incremental quantiles: q10=%.6f, q90=%.6f",
                    q10_val, q90_val
                )
    ...
    if 'atr' not in df.columns:
        ...
        import numpy as np
        high_arr = df['high'].values if hasattr(df['high'], 'values') else np.asarray(df['high'], dtype=np.float32)
        low_arr = df['low'].values if hasattr(df['low'], 'values') else np.asarray(df['low'], dtype=np.float32)
        close_arr = df['close'].values if hasattr(df['close'], 'values') else np.asarray(df['close'], dtype=np.float32)
