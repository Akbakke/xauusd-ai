#!/usr/bin/env bash
# ENTRY_V10.1 Threshold Sweep Test 2025
#
# Automatized sweep test for multiple thresholds to find optimal entry threshold.
# Tests thresholds: [0.18, 0.22, 0.25, 0.28]
#
# For each threshold:
#   - Creates config file
...
#   bash scripts/run_entry_v10_1_threshold_sweep_2025.sh [--parallel]
...
# Function to create config for a threshold
create_threshold_config() {
    ...
    # Create entry config (based on THRESHOLD_030 template)
    cat > "$entry_config" << EOF
# ENTRY_V10.1_SNIPER Policy Configuration (FLAT THRESHOLD ${threshold}) - Threshold sweep
#
# SNIPER entry policy with ENTRY_V10.1 HYBRID (XGB + Transformer).
# FLAT mode: No aggressive sizing, uses baseline sizing from P4.1.
# THRESHOLD ${threshold}: Threshold sweep test value.
#
# Configuration:
...
#
# Status: OFFLINE_ONLY (threshold sweep test)

entry_v9_policy_sniper:
  enabled: true
  min_prob_long: ${threshold}  # Threshold sweep test value
...
entry_gating:
  p_side_min:
    long: ${threshold}  # Threshold sweep test value
    short: 1.0
  margin_min:
    long: 0.50
    short: 0.50
...
EOF
    ...
    # Create main config (based on THRESHOLD_030 template)
    cat > "$main_config" << EOF
...
# Function to extract metrics from trade journal
extract_metrics() {
    local output_dir=$1
    local threshold=$2
...
# Function to run replay for a threshold
...
" "$policy_config" "$chunk_file" "$chunk_output" > "$output_dir/logs/chunk_${i}.log" 2>&1
    ...
    local report_path="reports/rl/entry_v10/ENTRY_V10_1_FLAT_${threshold_str}_FULLYEAR.md"
    python3 << PYEOF
import json
from pathlib import Path
...
if 'error' in metrics:
    lines = [
        f'# ENTRY_V10.1 FLAT THRESHOLD {threshold} FULLYEAR 2025 Replay Results',
        '',
...
# Run replays
echo "[RUN] Running replays $([ "$PARALLEL" = true ] && echo "in parallel" || echo "sequentially")..."
if [ "$PARALLEL" = true ]; then
    for threshold in "${THRESHOLDS[@]}"; do
        run_threshold_replay "$threshold" > "/tmp/threshold_${threshold}_replay.log" 2>&1 &
    done
    wait
    ...
    for threshold in "${THRESHOLDS[@]}"; do
        ...
        run_threshold_replay "$threshold" 2>&1 | tee "/tmp/threshold_${threshold}_replay.log"
        echo ""
    done
    echo "✅ All replays completed"
fi
...
echo ""
echo "=================================================================================="
echo "Generating combined summary report"
echo "=================================================================================="
echo ""
...
for threshold in thresholds:
    ...
    if output_base.exists():
        runs = sorted([d for d in output_base.iterdir() if d.is_dir()], key=lambda x: x.stat().st_mtime, reverse=True)
        if runs:
...
report_path = Path('reports/rl/entry_v10/ENTRY_V10_1_THRESHOLD_SWEEP_SUMMARY_2025.md')

lines = [
    '# ENTRY_V10.1 THRESHOLD SWEEP SUMMARY 2025',
    '',
    f'**Date:** {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}',
    '',
    '**Purpose:** Find optimal entry threshold for ENTRY_V10.1 FLAT configuration',
    '',
    '## Comparison Table',
    '',
...
lines.extend([
    ...
    '1. **Trade Count**: Higher threshold = fewer trades (higher selectivity)',
...
print(f'✅ Combined summary saved to: {report_path}')
PYEOF

echo ""
echo "=================================================================================="
echo "✅ THRESHOLD SWEEP TEST COMPLETE"
echo "=================================================================================="
echo ""
echo "Summary report: reports/rl/entry_v10/ENTRY_V10_1_THRESHOLD_SWEEP_SUMMARY_2025.md"
echo ""
