...
AUDIT_CAP=("$ENG/scripts/gx1_capped_run.sh" --class audit --mem 4G --swap 512M --)
...
PRETEST_ONLY=0
PRETEST_PAIR_JSON=
PRETEST_M1_SOURCE_MANIFEST=
PRETEST_M5_SOURCE_MANIFEST=
...
usage() {
  printf '%s\n' \
    ...
    "  TRAIN/VAL-only recovery: --pretest-only replaces tape/pair-generation/TEST authority arguments with" \
    "  --m1-lifecycle-pretest-pair-json PATH --m1-lifecycle-pretest-source-manifest PATH" \
    "  --pretest-m5-quote-source-manifest PATH; no TEST path/window is permitted."
}
...
while (($#)); do
  case "$1" in
    ...
    --pretest-only) PRETEST_ONLY=1; shift ;;
    --m1-lifecycle-pretest-pair-json) PRETEST_PAIR_JSON=${2:-}; shift 2 ;;
    --m1-lifecycle-pretest-source-manifest) PRETEST_M1_SOURCE_MANIFEST=${2:-}; shift 2 ;;
    --pretest-m5-quote-source-manifest) PRETEST_M5_SOURCE_MANIFEST=${2:-}; shift 2 ;;
    -h|--help) usage; exit 0 ;;
...
required_values=(
  ...
  MTF_CACHE_DIR M1_FEATURE_BASE_PARQUET M5_FEATURE_BASE_PARQUET EXIT_LIFECYCLE_DIR
  OUTPUT AUDIT_OUT_DIR HISTORY_START TRAIN_START TRAIN_END VAL_START VAL_END
)
if [[ $PRETEST_ONLY -eq 1 ]]; then
  # Reject mixed authority before any file check, path resolution or mkdir.
  for name in TAPE_ROOT M1_LIFECYCLE_PAIR_MANIFEST_JSON M1_LIFECYCLE_PAIR_GENERATION_ROOT REBUILD_TERMINAL_JSON PREFREEZE_TEST_SEAL_JSON TEST_START TEST_END; do
    if [[ -n ${!name} ]]; then
      printf '[ABORT] --pretest-only forbids legacy/TEST argument: %s\n' "$name" >&2
      exit 2
    fi
  done
  [[ $RESUME_EXACT_CHECKPOINTS -eq 0 ]] || { echo '[ABORT] pretest recovery requires fresh outputs, not resume' >&2; exit 2; }
  required_values+=(PRETEST_PAIR_JSON PRETEST_M1_SOURCE_MANIFEST PRETEST_M5_SOURCE_MANIFEST)
else
  for name in PRETEST_PAIR_JSON PRETEST_M1_SOURCE_MANIFEST PRETEST_M5_SOURCE_MANIFEST; do
    [[ -z ${!name} ]] || { echo '[ABORT] pretest inputs require --pretest-only' >&2; exit 2; }
  done
  required_values+=(TAPE_ROOT M1_LIFECYCLE_PAIR_MANIFEST_JSON M1_LIFECYCLE_PAIR_GENERATION_ROOT REBUILD_TERMINAL_JSON PREFREEZE_TEST_SEAL_JSON TEST_START TEST_END)
fi
...
if [[ $PRETEST_ONLY -eq 1 ]]; then
  bash "$ENG/scripts/gx1_handover.sh" --source-only
fi
...
required_files=(SOURCE_PARQUET CANONICAL_V2_PARQUET SIGNAL_MANIFEST FEATURE_RANKING_JSON M1_FEATURE_BASE_PARQUET M5_FEATURE_BASE_PARQUET)
if [[ $PRETEST_ONLY -eq 1 ]]; then
  required_files+=(PRETEST_PAIR_JSON PRETEST_M1_SOURCE_MANIFEST PRETEST_M5_SOURCE_MANIFEST)
else
  required_files+=(M1_LIFECYCLE_PAIR_MANIFEST_JSON)
fi
...
if [[ ! -d $MTF_CACHE_DIR ]] || { [[ $PRETEST_ONLY -eq 0 ]] && [[ ! -d $TAPE_ROOT || ! -d $M1_LIFECYCLE_PAIR_GENERATION_ROOT ]]; }; then
...
if [[ $PRETEST_ONLY -eq 1 ]]; then
  FULL_INPUT_LIVENESS_JSON="$OUTPUT_DIR/ENTRY_FULL_INPUT_LIVENESS_CONTRACT_${FULL_INPUT_LIVENESS_TIMESTAMP}.json"
fi
...
if [[ $PRETEST_ONLY -eq 0 ]]; then BUILD_SPLITS+=(test); fi
for split in "${BUILD_SPLITS[@]}"; do
...
"${CAP[@]}" "$PY" - "$SIGNAL_MANIFEST" "$FEATURE_RANKING_JSON" "$RUN_ID" "$SOURCE_PARQUET" \
    ...
    "$TRAIN_START" "$TRAIN_END" "$PRETEST_ONLY" <<'PY'
import hashlib
import json
import sys
...
with source_path.open("rb") as handle:
    for chunk in iter(lambda: handle.read(1024 * 1024), b""):
        digest.update(chunk)
expected_time_max = sys.argv[8]
expected_train_end = sys.argv[10]
if sys.argv[11] == "1":
    import pandas as pd
    import pyarrow.parquet as pq
    clock = pd.DatetimeIndex(pq.read_table(source_path, columns=["time"]).column("time").to_pandas())
...
lineage = validate_signal_manifest_training_lineage(
    ...
    expected_time_max_utc=expected_time_max,
    expected_train_start_utc=sys.argv[9],
    expected_train_end_utc=expected_train_end,
)
...
"${AUDIT_CAP[@]}" "$PY" -m gx1.scripts.audit_entry_cross_surface_feature_overlap_v1 \
  --run-id "$RUN_ID" \
...
if [[ $PRETEST_ONLY -eq 1 ]]; then
  BUILD_END=$VAL_END
  AUTHORITY_ARGS=(--pretest-only
    --m1-lifecycle-pretest-pair-json "$PRETEST_PAIR_JSON"
    --m1-lifecycle-pretest-source-manifest "$PRETEST_M1_SOURCE_MANIFEST"
    --pretest-m5-quote-source-manifest "$PRETEST_M5_SOURCE_MANIFEST")
else
  BUILD_END=$TEST_END
  AUTHORITY_ARGS=(--tape_root "$TAPE_ROOT"
...
"${CAP[@]}" "$PY" -m gx1.scripts.build_entry_v10_ctx_training_dataset_v3 \
  ...
  --output "$OUTPUT" \
  --start "$HISTORY_START" --end "$BUILD_END" \
...
"${AUDIT_CAP[@]}" "$PY" -m gx1.scripts.materialize_entry_full_input_liveness_v1 \
  --run-id "$RUN_ID" \
...
"${AUDIT_CAP[@]}" "$PY" - "$FULL_INPUT_LIVENESS_JSON" "$OUTPUT_DIR" <<'PY'
import json
import sys
...
PRETRAIN_ARGS=()
if [[ $PRETEST_ONLY -eq 1 ]]; then
  # Full real population, derived from the two newly emitted parquet headers.
  # No TEST path is constructed or inspected in this branch.
  MAX_SPLIT_ROWS=$("$PY" - "$OUTPUT_DIR" "$OUTPUT_STEM" <<'PY'
import sys
from pathlib import Path
import pyarrow.parquet as pq

rows = [pq.ParquetFile(Path(sys.argv[1]) / f"{sys.argv[2]}_{split}.parquet").metadata.num_rows for split in ("train", "val")]
if min(rows) <= 0:
    raise RuntimeError("PRETEST_REBUILD_EMPTY_SPLIT")
print(max(rows))
PY
)
  PRETRAIN_ARGS=(--max-rows-per-split "$MAX_SPLIT_ROWS" --max-row-groups-per-split 0)
fi
"${AUDIT_CAP[@]}" "$PY" -m gx1.scripts.audit_xau_direction_repair_pretrain_v1 \
  "${PRETRAIN_ARGS[@]}" \
  --dataset-dir "$OUTPUT_DIR" \
  --stem "$OUTPUT_STEM" \
