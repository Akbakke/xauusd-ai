  ...
  case "$1" in
    ...
    --smart-seq520)
      ...
      if [[ -z "${ENTRY_FOUNDATION_SMOKE_DIRECTION_CE_SCALE+x}" ]]; then
        SMOKE_DIRECTION_CE_SCALE=2.00
      else
        echo "WARNING: smart auto-recipe DIRECTION_CE_SCALE=2.00 overridden by env: ${SMOKE_DIRECTION_CE_SCALE}" >&2
      fi
      ...
      if [[ -z "${ENTRY_FOUNDATION_SMOKE_DIRECTION_MIN_PRED_RATE_LOSS_WEIGHT+x}" ]]; then
        SMOKE_DIRECTION_MIN_PRED_RATE_LOSS_WEIGHT=2.50
      else
        echo "WARNING: smart auto-recipe DIRECTION_MIN_PRED_RATE_LOSS_WEIGHT=2.50 overridden by env: ${SMOKE_DIRECTION_MIN_PRED_RATE_LOSS_WEIGHT}" >&2
      fi
      ...
      if [[ -z "${ENTRY_FOUNDATION_SMOKE_DIRECTION_MIN_PRED_RATE_SOFTMAX_TEMPERATURE+x}" ]]; then
        SMOKE_DIRECTION_MIN_PRED_RATE_SOFTMAX_TEMPERATURE=0.20
      else
        echo "WARNING: smart auto-recipe DIRECTION_MIN_PRED_RATE_SOFTMAX_TEMPERATURE=0.20 overridden by env: ${SMOKE_DIRECTION_MIN_PRED_RATE_SOFTMAX_TEMPERATURE}" >&2
      fi
      ...
      if [[ -z "${ENTRY_FOUNDATION_SMOKE_FLAT_CLASS_WEIGHT_FLOOR+x}" ]]; then
        SMOKE_FLAT_CLASS_WEIGHT_FLOOR=2.00
      fi
