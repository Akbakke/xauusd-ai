...
echo "resume_stage: CPU_ARTIFACT_PREFLIGHT_THEN_BOUNDED_GATE_LEARNING_VALIDATION_THEN_REPEAT_AUDIT"
...
echo "  1. run CPU source, prediction-artifact and smoke-bundle preflight against the frozen successful VAL artifact"
echo "  2. run only a predeclared bounded guarded learning-validation probe if CPU preflight passes"
echo "  3. repeat the smoke-bundle audit; a smoke metric is not an edge claim"
echo "  4. materialize candidate readiness only after that audit passes; then plan full candidate training"
echo "  5. report historical VAL/untouched-TEST research metrics with no production edge claim"
