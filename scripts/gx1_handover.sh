...
echo "dataset_contract: V40_GREEN_FOUNDATION_AUDITS_PASS_ADMISSION_PENDING"
...
echo "resume_stage: ANALYZE_V40_POWER_CEILING_BEFORE_ANY_HISTORICAL_CUDA_RETRY"
echo "dataset_rebuild: NOT_REQUIRED_CURRENT_V40_BYTES_PASS"
...
echo "  1. no automatic historical CUDA retry: the batch-8 diagnostic stopped at 180 W"
echo "  2. establish an independently measurable host-side power plan before another diagnostic"
echo "  3. only then assess a separately audited train recipe -> smoke bundle audit -> candidate readiness"
