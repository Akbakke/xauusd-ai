...
echo "resume_stage: GUARDED_VAL_PREDICTIONS_THEN_SMOKE_BUNDLE_AUDIT_THEN_CANDIDATE_READINESS"
...
echo "local_cuda: EXACT_VAL_EVALUATOR_ONLY_BEHIND_220W_70C_12G_GUARD"
...
echo "  1. produce immutable V46 VAL prediction evidence only with the exact guarded CUDA evaluator"
echo "  2. run the smoke-bundle audit; a smoke metric is not an edge claim"
echo "  3. materialize candidate readiness only after that audit passes; then plan full candidate training"
echo "  4. report historical VAL/untouched-TEST research metrics with no production edge claim"
