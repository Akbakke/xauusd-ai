...
echo "candidate_static_gate_source_policy: PROVISIONAL_POSITIVE_OPEN_ONLY__FULL_RAW_LIVENESS_DIRECT_INPUT_AND_ENTRY_FAMILY_PROOFS_REQUIRED"
...
echo "source_regression_last_verified: targeted parity/journal/gate tests must pass on the frozen repair commit; no whole-repository green claim is made here"
...
echo "resume_stage: COMMIT_FOCUSED_PARITY_JOURNAL_GATE_REGRESSIONS_THEN_DECLARE_ONE_GUARDED_CANDIDATE_LEARNING_VALIDATION"
...
echo "  1. run focused source regressions for CPU checkpoint parity, VAL journal and static-gate fail-closed policy"
echo "  2. commit the exact repair and rerun this handover from a clean worktree"
echo "  3. declare only one bounded guarded candidate learning-validation probe; it must re-run full raw liveness and candidate VAL evidence"
echo "  4. repeat candidate audit; a technical or candidate VAL metric is not an edge claim"
