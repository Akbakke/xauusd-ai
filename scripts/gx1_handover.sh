...
echo "train_recipe: V9_FULL_TECHNICAL_TRAIN_VAL_FROZEN__CURRENT_SOURCE_CPU_DRY_RUN_ONLY__NO_CUDA_OR_PROMOTION_AUTHORITY"
...
echo "fresh_31004_train: BLOCKED_PENDING_160W_PHYSICAL_LIMIT_SET_REPROBE_CLEAN_PREFLIGHT_AND_EXPLICIT_REAUTHORIZATION"
...
echo "host_telemetry: POST_RESTART_SIGNED_ENDPOINT_PASS__CONFIGURED_PHYSICAL_LIMIT_390W__160W_SET_REPROBE_REQUIRED"
echo "current_cuda_authority: BLOCKED_PENDING_160W_PHYSICAL_LIMIT_SET_REPROBE_CLEAN_PREFLIGHT_AND_EXPLICIT_REAUTHORIZATION"
...
echo "  2. from native elevated Windows PowerShell set the physical limit to 160 W, then obtain a fresh signed bridge response proving it; do not launch CUDA while it reports 390 W"
echo "  3. review the negative V9 VAL result and complete only CPU/read-only audits until an exact new recipe and explicit CUDA authorisation exist"
