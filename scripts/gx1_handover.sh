...
echo "source_regression_last_verified: targeted V46 liveness/model-routing/capped-run tests passed before the 2026-08-28 thermal-run record; no whole-repository green claim is made here"
...
echo "resume_stage: ONE_APPROVED_LOCAL_BATCH8_220W_SMOKE_THEN_REASSESS_THEN_REMOTE_OR_CANDIDATE"
...
echo "local_cuda: ONE_APPROVED_BATCH8_220W_CANONICAL_SMOKE_ONLY_THEN_REASSESS"
...
echo "  1. run exactly one fresh V46 batch-8 canonical smoke behind the 220 W guard; then reassess its terminal evidence"
echo "  2. use an approved remote V46 canonical smoke if that local smoke cannot create a valid bundle safely"
echo "  3. report historical VAL/untouched-TEST research metrics with no production edge claim"
echo "  4. bind immutable broker costs, financing, gap/terminal treatment and portfolio capital before demo, paper, live or production-net claims"
