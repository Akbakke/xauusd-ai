...
echo "candidate_static_gate_runtime_evidence: NOT_YET_RUN"
echo "external_full_training: NO_GO_PENDING_FRESH_CANDIDATE_EVIDENCE"
...
echo "resume_stage: COMMIT_SOURCE_BINDING_REPAIR_THEN_DECLARE_ONE_GUARDED_CANDIDATE_PARTIAL_SESSION"
...
echo "local_cuda: PREDECLARED_20MIN_CANDIDATE_PARTIAL_SESSION_ONLY_BEHIND_220W_70C_12G_GUARD"
...
echo "  1. run focused source regressions for model/VAL/cache/lifecycle bindings and Exit-only static-gate policy"
echo "  2. commit the exact repair and rerun this handover from a clean worktree"
echo "  3. materialize a fresh recipe and pass its no-GPU dry-run"
echo "  4. declare only one bounded 20-minute candidate session; it stops before first VAL and proves resumability only"
echo "  5. repeat candidate audit only after a completed full candidate; no partial-session metric is an edge claim"
echo "  6. run preregistered untouched-TEST evaluation only after the candidate/OOS gates, never as a troubleshooting input"
echo "  7. bind immutable broker costs, financing, gap/terminal treatment and portfolio capital before demo, paper, live or production-net claims"
