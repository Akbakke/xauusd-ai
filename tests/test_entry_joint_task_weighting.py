...
def _episode_fixture() -> dict[str, object]:
    return {
        ...
        "exit_action_valid_mask": torch.ones(2, 2, 2, dtype=torch.bool).numpy(),
        "exit_state_valid_mask": torch.ones(2, 2, dtype=torch.bool).numpy(),
    }
...
def _patch_episode_owners(monkeypatch: pytest.MonkeyPatch, targets: torch.Tensor):
    def forward(*, model, entry_decision_representation, episode, **_kwargs):
        ...
        state_valid = torch.ones_like(q[..., 0], dtype=torch.bool)
    ...
    monkeypatch.setattr(
        trainer, "_fitted_q_targets_for_episode_batch", fitted_targets
    )
...
def test_full_exit_streaming_matches_monolithic_objective_and_retains_ties(
    ...
    entry_gradient, stats = trainer._train_unified_exit_full_population(
        model=streamed,
    ...
    assert stats["population_rows"] == 4
    assert stats["q_valid_cells"] == 8
    assert stats["target_equivalent_action_rows"] == 1
    assert streamed.forwarded_rows == 4
...
def test_full_exit_validation_uses_population_and_valid_denominators(
    ...
    raw_loss, stats = trainer._unified_exit_full_population_eval_loss(
        model=model,
        ...
        entry_row_indices=torch.tensor([7]),
        dataset=_ExitDataset(_episode_fixture()),
        device=torch.device("cpu"),
    )
    ...
    assert stats["q_valid_cells"] == 8
    assert stats["target_equivalent_action_rows"] == 1
    assert stats["hold_target_greedy_rows"] == 2
    assert stats["exit_now_target_greedy_rows"] == 3
    assert model.forwarded_rows == 4
    assert float(raw_loss) == pytest.approx(stats["raw_loss"])


def test_joint_task_formula_omits_absent_task_entirely() -> None:
    model = _TaskWeights()
    model.task_log_variances["direction"].data.fill_(0.3)
    raw = torch.tensor(2.0, requires_grad=True)

    loss, stats = trainer._joint_task_loss(model, {"direction": raw})
    expected = torch.exp(torch.tensor(-0.3)) * 2.0 + 0.3
    assert torch.allclose(loss, expected)
    assert stats["joint_task_raw_loss_direction"] == 2.0

    loss.backward()
    assert model.task_log_variances["direction"].grad is not None
    assert model.task_log_variances["unified_exit_action"].grad is None
