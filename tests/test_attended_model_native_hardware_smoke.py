...
def test_hardware_smoke_builds_exact_shape_contract_without_reading_market_data() -> None:
    ...
    assert tuple(batch["seq_x"].shape) == (8, 96, 240)
    assert tuple(batch["snap_x"].shape) == (8, 240)
    assert tuple(batch["ctx_cont"].shape) == (8, 71)
    assert tuple(batch["ctx_cat"].shape) == (8, 1)
    assert tuple(batch["seq_m15"].shape) == (8, 64, 178)
    assert tuple(batch["seq_h1"].shape) == (8, 96, 178)
    assert tuple(batch["seq_h4"].shape) == (8, 96, 178)
    assert tuple(batch["seq_d1"].shape) == (8, 252, 178)
