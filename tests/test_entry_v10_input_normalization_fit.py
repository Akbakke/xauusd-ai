...
def _mtf_values(rows: int) -> np.ndarray:
    ...
    column = np.arange(len(MULTI_TF_PER_BAR_FEATURES_V2), dtype=np.float32)[
        None, :
    ]
    values = (
    ...
    ema_stack = list(MULTI_TF_PER_BAR_FEATURES_V2).index("ema_stack_aligned_v2")
    regime = list(MULTI_TF_PER_BAR_FEATURES_V2).index("regime_class_id")
...
def _mtf_frame(
    ...
    *,
    ...
    frame.attrs["feats_np"] = np.ascontiguousarray(values, dtype=np.float32)
...
def _train_arrays(
    *,
    ...
    times = pd.date_range(
        "2026-02-15T00:00:00Z", periods=rows, freq="1D", tz="UTC"
    )
...
def _artifacts(
    ...
    *,
    ...
    m5_prebuilt.write_bytes(b"immutable synthetic XAU M5 identity")

    source_times = pd.date_range(
        "2025-11-01T00:00:00Z", periods=150, freq="1D", tz="UTC"
    ).asi8
    source_values = _mtf_values(len(source_times))
    features = {
        tf: _mtf_frame(source_times, source_values + np.float32(offset * 0.01))
    ...
    for frame in features.values():
        frame.attrs["feats_np"][:, ema_stack] = (
            np.arange(len(source_times)) % 3 - 1
        ).astype(np.float32)
        frame.attrs["feats_np"][:, regime] = (
            np.arange(len(source_times)) % 5
        ).astype(np.float32)
    ...
    manifest = {
        ...
        "extra": {"entry_run_id": "SYNTHETIC_XAU_TRAIN"},
        "feature_contract": {
...
def test_overlapping_mtf_windows_fit_each_source_row_once() -> None:
    ...
    assert selected.shape == (4, len(MULTI_TF_PER_BAR_FEATURES_V2))
...
def test_full_train_fit_uses_snap_once_and_binds_all_lineage(
    fit_fixture: dict,
) -> None:
    ...
    assert contract["surfaces"]["signal"]["fit_row_count"] == rows
    assert contract["surfaces"]["ctx_cont"]["fit_row_count"] == rows
    assert contract["ctx_cat"]["fit_row_count"] == rows
    ...
    assert contract["surfaces"]["mtf_m5"]["field_count"] == 25
    assert contract["surfaces"]["mtf_m5"]["field_names"] == list(
        MULTI_TF_PER_BAR_FEATURES_V2
    )
    assert proof["train_decision_row_count"] == rows
    assert proof["sequence_population"].endswith("not_flattened_for_fit")
    assert proof["val_fit_row_count"] == proof["test_fit_row_count"] == 0

    non_alias_index = 0
    expected_median = np.float32(
        np.median(fit_fixture["snap"][:, non_alias_index])
    )
    assert (
...
def test_alias_stats_are_ctx_owned_and_dynamic(fit_fixture: dict) -> None:
