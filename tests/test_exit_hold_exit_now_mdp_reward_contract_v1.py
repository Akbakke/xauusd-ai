...
def test_write_artifacts_produces_required_outputs(tmp_path: Path) -> None:
    out_root = tmp_path / "EXIT_HOLD_EXIT_NOW_MDP_REWARD_CONTRACT_V1_20260429T999999Z_LOCK"
    ...
    for required in [
        ...
        "status_v1.json",
        "report_v1.md",
        "exit_hold_exit_now_mdp_reward_contract_go_no_go_v1.json",
        "input_manifest_v1.json",
        "mdp_contract_v1.json",
        "action_contract_v1.json",
        "reward_contract_v1.json",
        "transition_semantics_v1.json",
        "discount_factor_lock_v1.json",
        "state_schema_requirements_v1.json",
        "no_shortcut_axioms_v1.json",
        "action_support_requirement_v1.json",
        "pre_train_dependency_graph_v1.json",
        "mdp_self_consistency_audit_v1.json",
    ]:
    ...
    assert summary["self_consistency_status_v1"] == "PASS"
    assert summary["action_set_v1"] == ["HOLD", "EXIT_NOW"]
    assert summary["hold_reward_scheme_v1"] == "ZERO_IMMEDIATE_TERMINAL_ONLY"
    assert summary["terminal_reward_variant_count_v1"] == 6
    assert summary["trainable_reward_variant_count_v1"] == 5
    go_no_go = json.loads(
        (artifact_root / "exit_hold_exit_now_mdp_reward_contract_go_no_go_v1.json").read_text()
    )
    assert go_no_go["training_allowed_v1"] is False
    assert go_no_go["iql_production_allowed_v1"] is False
