...
def _source_fixture(tmp_path: Path) -> tuple[Path, Path]:
    ...
    payload = {
        ...
        "rank_reference_sha256": _sha256_file(rank),
        "output_parquet": str(source),
        "output_parquet_sha256": _sha256_file(source),
    }
...
def test_feature_base_requires_exact_enriched_source_lineage(
    ...
    binding = _load_enriched_source_binding(
        source,
        dataset_run_id=DATASET_RUN_ID,
        pair_generation_id=PAIR_GENERATION_ID,
        timeframe="M1",
    ...
    assert binding["manifest_sha256"] == _sha256_file(manifest)
    assert binding["source_sha256"] == _sha256_file(source)
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("rank_reference_sha256", "0" * 64, "SOURCE_RANK_LINEAGE_INVALID"),
    ),
)
