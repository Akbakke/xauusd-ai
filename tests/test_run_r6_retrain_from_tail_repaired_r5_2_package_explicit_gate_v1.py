...
from gx1.scripts import materialize_run_r6_retrain_from_tail_repaired_r5_2_package_explicit_gate_v1 as r6
...
def _seed_package(root: Path, *, integrity_status: str = "PASS") -> None:
    for name in r6.REQUIRED_PACKAGE_FILES:
        ...
        elif name == "tail_repaired_r5_2_candidate_package_manifest_v1.json":
            _write_json(path, {"final_promotion_allowed_v1": False})
        elif name == "tail_repaired_r5_2_candidate_package_integrity_report_v1.json":
            _write_json(path, {"status_v1": integrity_status})
        elif name == "tail_repaired_r5_2_candidate_package_r6_input_readiness_precheck_v1.json":
            _write_json(path, {"status_v1": "R6_INPUT_PACKAGE_READY_BUT_R6_NOT_AUTHORIZED"})
        elif name == "summary_v1.json":
            _write_json(
                path,
                {
                    "selected_candidate_id_v1": "TAIL_TARGET_WEIGHT_REPAIR_CONSERVATIVE::RECALL",
...
def test_r6_cannot_run_if_package_provenance_missing(tmp_path: Path) -> None:
    ...
    (package / "tail_repaired_r5_2_candidate_oof_score_provenance_v1.csv").unlink()
...
def test_r6_candidate_grid_includes_pass_through_control() -> None:
    ...
    assert any(row["candidate_id_v1"] == "TAIL_REPAIRED_R5_2_PASS_THROUGH_CONTROL" for row in grid)


def test_r6_candidate_grid_is_not_optuna_or_large_sweep() -> None:
    assert len(r6._candidate_grid()) <= 8
    too_large = [{"candidate_id_v1": "TAIL_REPAIRED_R5_2_PASS_THROUGH_CONTROL"}]
...
def test_low_support_groups_remain_visible_in_best_status() -> None:
    ...
    assert status == "R6_TAIL_REPAIRED_CANDIDATE_PRESERVES_140_94_WITH_STRONGER_HEAD_DIAGNOSTICS"
...
def test_tail_repaired_package_validation_rejects_wrong_selected_candidate(tmp_path: Path) -> None:
    package = tmp_path / "package"
    _seed_package(package)
    summary = json.loads((package / "summary_v1.json").read_text(encoding="utf-8"))
    summary["selected_candidate_id_v1"] = "TAIL_REPAIR_MAX_SAFE_DIAGNOSTIC::RECALL"
    _write_json(package / "summary_v1.json", summary)
    with pytest.raises(RuntimeError, match="R6_INPUT_PACKAGE_VALIDATION_FAILED"):
        r6.validate_input_package(package)


def test_tail_repaired_fixed_controls_include_previous_package_and_wednesday() -> None:
    ids = {row["control_v1"] for row in r6.FIXED_CONTROLS}
    assert "tail_repaired_r5_2_package_pass_through" in ids
    assert "previous_r5_2_package" in ids
    assert "previous_r6_pass_through" in ids
    assert "wednesday_benchmark" in ids


def test_tail_repaired_grid_includes_tail_focused_hard_veto_candidate() -> None:
    ids = {row["candidate_id_v1"] for row in r6._candidate_grid()}
    assert "R6_TAIL_FOCUSED_WITH_HARD_VETO" in ids


def test_tail_repaired_status_names_do_not_authorize_r6_package_on_weaker_candidate() -> None:
    best = {
        "bad_count_v1": 139,
        "tail_count_v1": 94,
        "safety_clean_v1": True,
    }
    status, next_action = r6._best_status(best, provenance_ok=True, no_in_sample_ok=True, no_overlap_ok=True)
    assert status == "R6_TAIL_REPAIRED_CANDIDATE_SAFE_BUT_WEAKER_THAN_140_94"
    assert next_action == "R6_HEAD_SIGNAL_AUDIT_OR_KEEP_TAIL_REPAIRED_R5_2_PACKAGE_AS_CONTROL_V1"
