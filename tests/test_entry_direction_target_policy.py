...
def test_entry_direction_manifest_binding_is_exact_and_split_shared() -> None:
    policy = entry_direction_target_policy_fixture()
    contract = direction_label_contract(policy)
    extra = {
        **contract,
        "early_move_threshold_bps": policy["early_move_threshold_bps"],
        "strict_entry_labels": {
            **contract,
            **hierarchical_direction_label_contract(),
        },
    }
    assert require_entry_direction_target_manifest_binding(
        extra,
        expected_source_parquet_sha256=policy["source_parquet_sha256"],
        expected_tape_provenance_sha256=policy["tape_provenance_sha256"],
        expected_train_start=policy["train_start_utc"],
        expected_train_end=policy["train_end_utc"],
    ) == policy

    mismatched = deepcopy(extra)
    mismatched["strict_entry_labels"][
        "entry_direction_target_policy_sha256"
    ] = "0" * 64
    with pytest.raises(RuntimeError, match="MANIFEST_PROJECTION_MISMATCH"):
        require_entry_direction_target_manifest_binding(mismatched)
