...
def test_joint_probe_covers_shared_exit_and_aux_preserves_state_and_cleans_errors(pair,monkeypatch,bad_target):
    class JointSmall(Small):
        def __init__(self):
            super().__init__();self.fuse=torch.nn.Linear(4,4);self.exit_linear=torch.nn.Linear(4,4);self.head_exit_action=torch.nn.Linear(4,2)
    ...
    def exit_loss(**kw):
        ...
        q=model.head_exit_action(model.exit_linear(token));raw=(q-.5).square().mean()
        ...
        return token.grad,stats,pair['target']+(1 if bad_target else 0),pair['valid']
    ...
    if bad_target:
        with pytest.raises(RuntimeError,match='TARGET_PARITY_INVALID'):runner._joint_update_probe(**args)
    else:
