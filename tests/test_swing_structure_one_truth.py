...
def test_entry_contract_is_the_only_context_subgroup_owner() -> None:
    ...
    # V30 (2026-08-14): the four derived regime/bucket categoricals are
    # retired; session_id is the only categorical owner left, and its width is
    # read from the contract rather than restated.
    assert MODEL_NATIVE_CTX_CAT_FIELDS == ("session_id",)
    assert MODEL_NATIVE_CTX_CAT_DIM == len(MODEL_NATIVE_CTX_CAT_FIELDS)
...
def test_builder_artifact_field_owners_are_exact_and_disjoint() -> None:
    ...
    assert set(source_owned) == (
        ...
        | (
            set(MODEL_NATIVE_CTX_CONT_REGIME_FIELDS)
...
def test_retired_rank_bucket_state_is_fail_closed() -> None:
    # The TRAIN-rank bucket subsystem (bucket_against_train_reference,
    # TrainRankReferenceV2, the atr_bucket/spread_bucket categoricals) is
    # retired: no rank reference is fitted and no bucket categorical is
    # emitted.  The state owner refuses any state that still carries a rank
    # field, and the categorical contract no longer names a bucket.
    valid = {
        "schema_version": MODEL_NATIVE_STATE_SCHEMA_VERSION,
        "feature_history_start_utc": "2020-01-01T00:00:00Z",
        "feature_history_mode": MODEL_NATIVE_HISTORY_MODE,
        "split_reset_allowed": False,
        "runtime_rule_free": True,
        "entry_run_id": "TEST_RUN",
    }
    assert validate_state_contract_metadata_v2(valid) == valid
    assert RETIRED_RANK_STATE_FIELDS
    for retired in sorted(RETIRED_RANK_STATE_FIELDS):
        with pytest.raises(
            RuntimeError, match="MODEL_NATIVE_STATE_RETIRED_FIELDS_FORBIDDEN"
        ):
            validate_state_contract_metadata_v2({**valid, retired: "x"})
    assert not [
        name for name in MODEL_NATIVE_CTX_CAT_FIELDS if name.endswith("_bucket")
    ]
