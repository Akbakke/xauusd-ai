...
def test_action_values_are_full_counterfactual_executable_pnl() -> None:
    frame = _spread_tape()
    targets, _ = _build_model_native_aux_head_targets(frame)
    ...
    assert targets["y_action_value_long_K12"][0] == pytest.approx(long_pnl)
...
def test_contract_forbids_feature_derived_core_target_rewrites() -> None:
    ...
    assert target["core_target_source"] == "future_executable_pnl_outcomes_only"
