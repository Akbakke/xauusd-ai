...
def _recipe(tmp_path: Path) -> dict[str, object]:
    ...
    names = {
        ...
        "unified_exit_lifecycle_v2_root_manifest": "UNIFIED_EXIT_LIFECYCLE_V2/root_manifest.json",
        "train_unified_exit_lifecycle_v2_parquet": "UNIFIED_EXIT_LIFECYCLE_V2/train.parquet",
        "train_unified_exit_lifecycle_v2_manifest": "UNIFIED_EXIT_LIFECYCLE_V2/train_manifest.json",
        "val_unified_exit_lifecycle_v2_parquet": "UNIFIED_EXIT_LIFECYCLE_V2/val.parquet",
        "val_unified_exit_lifecycle_v2_manifest": "UNIFIED_EXIT_LIFECYCLE_V2/val_manifest.json",
        "unified_exit_v2_economics_readiness": "UNIFIED_EXIT_LIFECYCLE_V2/economics_readiness.json",
        "train_unified_exit_v2_cost_authority": "UNIFIED_EXIT_LIFECYCLE_V2/train_cost_authority.json",
        "val_unified_exit_v2_cost_authority": "UNIFIED_EXIT_LIFECYCLE_V2/val_cost_authority.json",
...
def test_pretest_recipe_materializer_rejects_noncanonical_wrapper_before_inputs(
    ...
    with pytest.raises(RuntimeError, match="WRAPPER_PATH_INVALID"):
        materialize_pretest_technical_recipe(
            ...
            unified_exit_lifecycle_v2_root_manifest=tmp_path / "v2-root.json",
            train_unified_exit_lifecycle_v2_parquet=tmp_path / "v2-train.parquet",
            train_unified_exit_lifecycle_v2_manifest=tmp_path / "v2-train-manifest.json",
            val_unified_exit_lifecycle_v2_parquet=tmp_path / "v2-val.parquet",
            val_unified_exit_lifecycle_v2_manifest=tmp_path / "v2-val-manifest.json",
            unified_exit_v2_economics_readiness=tmp_path / "v2-readiness.json",
            train_unified_exit_v2_cost_authority=tmp_path / "v2-train-authority.json",
            val_unified_exit_v2_cost_authority=tmp_path / "v2-val-authority.json",
...
@pytest.mark.parametrize('selected', ['experimental_fp32_3090_finite_single', 'experimental_fp32_3090_finite_grouped'])
@pytest.mark.parametrize('change,valid', [({},True), ({'epochs':2},False), ({'grad_accum_steps':2},False), ({'subsample_rows':513},False), ({'batch_size':10},False), ({'execution_tier':'attended_only'},False)])
def test_local_finite_check_recipe_is_bounded_fp32_batch8(tmp_path, selected, change, valid):
    recipe = _recipe(tmp_path)
    cli = recipe['trainer_cli']
    cli.update(execution_tier='canonical', train_time_window=None, precision_policy=selected, batch_size=8, subsample_rows=512)
    cli.update(change)
    recipe['trainer_cli_sha256'] = canonical_json_sha256(cli)
    if valid:
        assert require_pretest_technical_recipe_metadata(recipe)['trainer_cli'] == cli
...
def test_kernel_profile_recipe_is_explicit_bounded_diagnostic(tmp_path, change, valid):
    recipe = _recipe(tmp_path)
    cli = recipe['trainer_cli']
    cli.update(execution_tier='canonical', train_time_window=None, precision_policy='experimental_fp32_3090_kernel_profile', batch_size=8, subsample_rows=512)
    cli.update(change)
    recipe['trainer_cli_sha256'] = canonical_json_sha256(cli)
    if valid:
        assert require_pretest_technical_recipe_metadata(recipe)['trainer_cli'] == cli
