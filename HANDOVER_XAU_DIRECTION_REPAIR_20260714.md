...
## 2026-07-16 Source Update - Direction Slice Confusion Pair Loss

- After `SMART_SEQ520_XAU_SMOKE_HIERTRADEACCEDGE_E6_20260716`, the best checkpoint had global class balance and no pred-rate slice failures, but still had `10` active slice accuracy failures. The next repair therefore targeted per-slice class confusion directly, not IQL and not more epochs.
- Implemented and committed `071efcea Require XAU direction slice confusion pair loss`.
  - Added `ENTRY_DIRECTION_SLICE_CONFUSION_PAIR_WEIGHT` and `ENTRY_DIRECTION_SLICE_CONFUSION_PAIR_MARGIN`.
  - The trainer now penalizes active ctx slices where average true-class probability fails to beat the worst active wrong-class probability by the configured margin.
  - Strict smart XAU repair requires weight `>=4.00` and margin `>=0.02`; missing or weak config fails preflight/audit/readiness.
  - The recipe is passed through smoke/candidate wrappers, rebuild defaults, smart readiness, smoke manifest, trainability, enablement, sweep lint, candidate/replay readiness, bundle audit, metadata, failure evidence, and tests.
  - This is not fallback. It is a hard model-training objective and a fail-closed contract.
- Validation before clean-git readiness:
  - `git diff --check` passed.
  - `py_compile` passed for changed Python modules.
...
- Clean-git post-commit gates passed:
  - `smart-smoke-readiness --quiet`
  - `smart-trainability-readiness --quiet`
  - `smart-smoke-train-enablement --vedtak SMART_SEQ520_XAU_SMOKE_DIRCONFPAIR_E6_20260716 --epochs 6 --batch-size 64 --quiet`
  - Enablement confirmed `ENTRY_DIRECTION_SLICE_CONFUSION_PAIR_WEIGHT=4.00`, `ENTRY_DIRECTION_SLICE_CONFUSION_PAIR_MARGIN=0.02`, and kept `candidate_training_allowed=false`, `iql_allowed=false`, `replay_allowed=false`, `promotion_shadow_live_allowed=false`, and `trainer_started=false`.
- Ran one bounded smoke:
  - Vedtak: `SMART_SEQ520_XAU_SMOKE_DIRCONFPAIR_E6_20260716`.
  - Pre-train manifest: `/home/andre2/GX1_DATA/reports/entry_foundation_smoke_train_manifests_20260628_v1/ENTRY_FOUNDATION_SMOKE_TRAIN_RUN_MANIFEST_20260716T053209Z.json`.
  - Intended bundle: `/home/andre2/GX1_DATA/runs/FASE2B_REGIME_V4_20260605/v10_6yr_rebuild_20260628_foundation_seq146/v10_entry_smart_seq520_smoke_20260716T053209Z`.
  - Failure evidence: `/home/andre2/GX1_DATA/runs/FASE2B_REGIME_V4_20260605/v10_6yr_rebuild_20260628_foundation_seq146/v10_entry_smart_seq520_smoke_20260716T053209Z__direction_slice_failure_evidence.json`.
  - Result: fail-closed on `[TRAIN_FAIL_DIRECTION_SLICE_GUARD]`; no bundle directory was written.
  - Best epoch was `1`: `dir_acc=0.365234`, `balance_guard_ok=1`, public pred LONG `0.397786`, SHORT `0.237630`, FLAT `0.364583`, `11` slice failures, `9` accuracy failures, `2` pred-rate failures, `direction_slice_ckpt_score=-0.090634`.
  - Best epoch hierarchy evidence: `trade_pred=0.952474`, `trade_prob=0.652458`, `side_acc_edge=0.528827`, `hier_flat_pred_rate=0.047526`.
  - Epoch `2` regressed and early-stopped: balance guard failed, public pred LONG `0.029297`, SHORT `0.637370`, FLAT `0.333333`, `29` slice failures (`12` accuracy, `17` pred-rate).
  - Interpretation: this source repair added a stricter hard objective and stopped cleanly, but it did not improve the best smoke beyond the prior hierarchy-trade-accuracy-edge run. Do not rerun `DIRCONFPAIR_E6` unchanged and do not extend epochs on it.
- Post-smoke resource state:
  - `/home/andre2/GX1_DATA`: about `838G` free.
  - RAM: about `35GiB` available, swap `0B`.
  - GPU idle at about `296MiB` reserved memory.
  - No active `python` or `python3` training/eval process.
- Current interpretation:
...
## 2026-07-16 Source Update - Hierarchy Public Side Head

- After `SMART_SEQ520_XAU_SMOKE_CTXDIRCAL_E6_20260716`, the next repair changed topology rather than scalar pressure:
  - public FLAT/no-trade remains composed from `trade_logit`;
  - public LONG/SHORT side-choice can now use a dedicated learned `head_public_side`;
  - hierarchy side/utility side heads are no longer forced to share the exact public side logits.
- Implemented and committed `596f1a67 Require XAU public side head`.
  - Added optional `enable_hierarchical_public_side_head` to `EntryV10CtxHybridTransformer`.
  - The model emits `public_side_logits` when the head is active.
  - Strict smart XAU repair now requires `ENTRY_HIER_PUBLIC_SIDE_HEAD=1`; wrappers, rebuild defaults, smoke/trainability readiness, smoke manifest, enablement, sweep lint, candidate/replay readiness, bundle audit, trainer metadata, and tests all carry the contract.
  - Bundle loading fails closed if `head_public_side.*` weights exist without public-side-head metadata.
  - This is not fallback. It is a model-native learned head and a hard contract.
- Validation before clean-git readiness:
  - `python3 -m py_compile` passed for changed Python modules.
  - `bash -n` passed for changed shell wrappers.
  - `git diff --check` passed.
...
- Clean-git post-commit gates passed:
  - `scripts/entry_next_edge_control.sh smart-smoke-readiness --quiet`.
  - `scripts/entry_next_edge_control.sh smart-trainability-readiness --quiet`.
  - `scripts/entry_next_edge_control.sh smart-smoke-train-enablement --vedtak SMART_SEQ520_XAU_SMOKE_PUBLICSIDE_ENABLEMENT_20260716 --epochs 6 --batch-size 64 --quiet`.
  - Enablement report: `/home/andre2/GX1_DATA/reports/entry_smart_seq520_smoke_train_enablement_20260715_v1/ENTRY_SMART_SEQ520_SMOKE_TRAIN_ENABLEMENT_20260716T060935Z.json`.
  - Enablement decision was `ENTRY_SMART_SEQ520_SMOKE_TRAIN_ENABLEMENT_READY_FOR_EXPLICIT_EXECUTION`; it kept `training_allowed=false`, `candidate_training_allowed=false`, `replay_allowed=false`, `iql_allowed=false`, `promotion_shadow_live_allowed=false`, and `trainer_started=false`.
- Ran one bounded smart smoke:
  - Vedtak: `SMART_SEQ520_XAU_SMOKE_PUBLICSIDE_E6_20260716`.
  - Command: `scripts/entry_next_edge_control.sh smart-smoke-train --vedtak SMART_SEQ520_XAU_SMOKE_PUBLICSIDE_E6_20260716 --require-edge-audit --epochs 6 --early-stop-patience 6`.
  - Pre-train manifest was written at `/home/andre2/GX1_DATA/reports/entry_foundation_smoke_train_manifests_20260628_v1/ENTRY_FOUNDATION_SMOKE_TRAIN_RUN_MANIFEST_20260716T060956Z.json`, then deleted after the run ended without a bundle or failure sidecar.
  - Intended bundle dir was not created: `/home/andre2/GX1_DATA/runs/FASE2B_REGIME_V4_20260605/v10_6yr_rebuild_20260628_foundation_seq146/v10_entry_smart_seq520_smoke_20260716T060956Z`.
  - No failure sidecar was written because the process did not reach the trainer's final fail-closed sidecar path.
  - Last observed validation at epoch `2`: balance guard OK, `dir_acc=0.367188`, public pred LONG `0.272786`, SHORT `0.224609`, FLAT `0.502604`, `14` slice failures (`11` accuracy, `3` pred-rate), `direction_slice_ckpt_score=-0.248524`, hierarchy `trade_pred=0.947917`, `trade_prob=0.619316`, `side_acc_edge=0.570577`.
  - Interpretation: the public-side-head topology is wired and contract-guarded, but the smoke evidence is still red. It is not a candidate and must not open IQL, replay, shadow, or live.
- Post-run resource and cleanup state:
  - No relevant `python`/`python3` training/eval process running.
  - `/home/andre2/GX1_DATA`: about `837G` free.
  - RAM: about `39GiB` available, swap `0B`.
  - The stale run manifest was deleted; no bundle dir, no failure sidecar, and no fresh memmap tmp dir existed.
- Current interpretation:
...
## 2026-07-16 Source Update - Public Margin Direction Composition

- After `SMART_SEQ520_XAU_SMOKE_PUBTRADE_E6_20260716`, the failure was structural:
  - public hierarchy used joint log-prob composition: `LONG=log P(trade)+log P(side long)`, `SHORT=log P(trade)+log P(side short)`, `FLAT=log P(flat)`;
  - with weak side confidence, FLAT wins argmax too easily even when trade probability is near target;
  - this explained the all-FLAT epoch-1 collapse and required a topology repair, not IQL and not scalar-only retuning.
- Implemented and committed `aba63709 Require XAU public margin composition`.
  - Added `ENTRY_HIER_PUBLIC_DIRECTION_COMPOSITION` with allowed values `logprob` and `margin`.
  - Strict smart XAU repair now requires `margin`.
  - In `margin` mode, public direction logits are `[public_trade_logit+public_side_long_logit, public_trade_logit+public_side_short_logit, -public_trade_logit]`.
  - Legacy/non-XAU default remains `logprob` for explicit compatibility, but smart XAU preflight/audit/readiness fails closed unless `margin` is present.
  - Bundle loading fails closed if a public trade/flat-from-trade bundle lacks public direction-composition metadata.
  - This is not fallback. It is a hard model topology contract.
- Validation before smoke:
  - `py_compile` passed for changed Python modules.
  - `bash -n` passed for changed shell wrappers.
  - `git diff --check` passed.
  - Deterministic model test proved margin logits exactly: trade bias `0.2`, side biases `[0.3,-0.1]` -> direction logits `[0.5,0.1,-0.2]`.
  - Focused entry/XAU pytest suite passed.
  - Broader XAU/entry/readiness suite passed, excluding only two stale legacy-import tests that reference inactive `gx1.execution` modules.
  - Pre-commit hook passed during commit.
- Clean-git post-commit gates passed:
  - `scripts/entry_next_edge_control.sh smart-smoke-readiness --quiet`
  - `scripts/entry_next_edge_control.sh smart-trainability-readiness --quiet`
  - `scripts/entry_next_edge_control.sh smart-smoke-train-enablement --vedtak SMART_SEQ520_XAU_SMOKE_MARGINCOMP_ENABLEMENT_20260716 --epochs 6 --batch-size 64 --quiet`
- Ran one bounded smoke and stopped it after epoch `1` because it was hard-red:
  - Vedtak: `SMART_SEQ520_XAU_SMOKE_MARGINCOMP_E6_20260716`.
  - Pre-train manifest was `/home/andre2/GX1_DATA/reports/entry_foundation_smoke_train_manifests_20260628_v1/ENTRY_FOUNDATION_SMOKE_TRAIN_RUN_MANIFEST_20260716T104820Z.json`, then deleted after manual stop because the run was aborted and stale.
  - Intended bundle dir was not created: `/home/andre2/GX1_DATA/runs/FASE2B_REGIME_V4_20260605/v10_6yr_rebuild_20260628_foundation_seq146/v10_entry_smart_seq520_smoke_20260716T104820Z`.
  - No matching fresh memmap/tmp dir was found.
  - Epoch `1`: `dir_acc=0.318359`, `balance_guard_ok=0`, `slice_contract_ok=0`, `51` slice failures (`17` accuracy, `34` pred-rate), `direction_slice_ckpt_score=-2.826244`.
  - Epoch `1` public pred rates: LONG `0.947266`, SHORT `0.048828`, FLAT `0.003906`.
  - Epoch `1` hierarchy evidence: `trade_pred=1.000000`, `trade_prob=0.509584`, `trade_prob_label_flat=0.509410`, `side_pred_long_edge=0.967197`, `side_acc_edge=0.492048`.
  - Interpretation: margin composition removed the all-FLAT log-prob starvation, but the model immediately collapsed to LONG. This is real progress on diagnosis/topology, not a usable candidate.
- Post-stop cleanup/resource state:
  - No transformer train/eval process running.
  - Aborted pre-train manifest deleted.
  - No bundle dir and no fresh memmap/tmp dir existed.
  - `/home/andre2/GX1_DATA`: about `837G` free; RAM about `38GiB` available, swap `0B`.
- Next action:
  1. Do not rerun `SMART_SEQ520_XAU_SMOKE_MARGINCOMP_E6_20260716` unchanged.
  2. Candidate/replay/IQL/shadow/live remain closed until a fresh XAU transformer bundle passes hard direction slice and class-balance gates.
  3. The next repair should target training staging/calibration/initialization around trade-vs-side learning under margin composition. Do not add fallback, do not move to IQL, and do not extend epochs on this red recipe.

## 2026-07-16 Source Update - Centered Public Margin Direction Composition

- After `SMART_SEQ520_XAU_SMOKE_MARGINCOMP_E6_20260716`, margin composition fixed the all-FLAT log-prob starvation shape but immediately collapsed to LONG.
  - The concrete structural issue was that raw public side logits still carried a common-mode component into the public LONG/SHORT logits.
  - That let the side head move both trade classes relative to FLAT even though public trade/no-trade should be controlled by `public_trade_logit`.
- Implemented and committed `cdd249bb Require XAU centered public margin composition`.
  - Added `ENTRY_HIER_PUBLIC_DIRECTION_COMPOSITION=margin_centered`.
  - In `margin_centered` mode, public side logits are mean-centered per row before composition:
    - `LONG=public_trade_logit+(public_side_long_logit-mean(side_logits))`
    - `SHORT=public_trade_logit+(public_side_short_logit-mean(side_logits))`
    - `FLAT=-public_trade_logit`
  - Strict smart XAU repair now requires `margin_centered`; old `margin` remains an explicit non-strict legacy value, not a valid XAU repair contract.
  - Bundle loader accepts and preserves the new metadata value; audit/readiness/sweep/wrappers/rebuild require it for XAU.
  - This is not fallback. It is a hard model-topology contract that separates side-choice from trade/no-trade common-mode.
- Validation before smoke:
  - `python3 -m py_compile` passed for changed Python modules.
  - `bash -n` passed for changed shell wrappers.
  - `git diff --check` passed.
  - Deterministic model tests passed, including proof that side biases `[1.3,0.9]` are centered to produce direction logits `[0.4,0.0,-0.2]` with trade bias `0.2`.
  - Focused XAU/entry/readiness pytest suite passed.
  - Broader XAU/entry/replay/readiness suite passed, excluding only the two stale legacy-import tests that reference inactive `gx1.execution` modules.
  - Pre-commit hook passed during commit.
- Clean-git post-commit gates passed:
  - `scripts/entry_next_edge_control.sh smart-smoke-readiness --quiet`
  - `scripts/entry_next_edge_control.sh smart-trainability-readiness --quiet`
  - `scripts/entry_next_edge_control.sh smart-smoke-train-enablement --vedtak SMART_SEQ520_XAU_SMOKE_MARGINCENTER_ENABLEMENT_20260716 --epochs 6 --batch-size 64 --quiet`
- Ran one bounded smoke and stopped it after epoch `1` because validation was still hard-red:
  - Vedtak: `SMART_SEQ520_XAU_SMOKE_MARGINCENTER_E6_20260716`.
  - Pre-train manifest was `/home/andre2/GX1_DATA/reports/entry_foundation_smoke_train_manifests_20260628_v1/ENTRY_FOUNDATION_SMOKE_TRAIN_RUN_MANIFEST_20260716T110042Z.json`, then deleted after manual stop because the run was aborted and stale.
  - Intended bundle dir was not created: `/home/andre2/GX1_DATA/runs/FASE2B_REGIME_V4_20260605/v10_6yr_rebuild_20260628_foundation_seq146/v10_entry_smart_seq520_smoke_20260716T110042Z`.
  - No matching fresh memmap/tmp dir was found.
  - Epoch `1`: `dir_acc=0.386719`, `balance_guard_ok=0`, `slice_contract_ok=0`, `20` slice failures (`6` accuracy, `14` pred-rate), `direction_slice_ckpt_score=-1.056852`.
  - Epoch `1` public pred rates: LONG `0.371094`, SHORT `0.555990`, FLAT `0.072917`; labels were LONG `0.322917`, SHORT `0.332031`, FLAT `0.345052`.
  - Epoch `1` hierarchy evidence: `trade_pred=1.000000`, `trade_prob=0.505739`, `trade_prob_label_flat=0.505617`, `side_pred_long_edge=0.520875`, `side_acc_edge=0.558648`.
  - Interpretation: centered margin composition removed the previous near-all-LONG side collapse and improved `dir_acc` and side balance, but FLAT/no-trade is still severely underpredicted and slice gates remain red. This is progress on topology, not a candidate.
- Post-stop cleanup/resource state:
  - No transformer train/eval process running.
  - Aborted pre-train manifest deleted.
  - No bundle dir and no fresh memmap/tmp dir existed.
  - `/home/andre2/GX1_DATA`: about `837G` free; RAM about `38GiB` available, swap `0B`.
- Next action:
  1. Do not rerun `SMART_SEQ520_XAU_SMOKE_MARGINCENTER_E6_20260716` unchanged.
  2. Candidate/replay/IQL/shadow/live remain closed until a fresh XAU transformer bundle passes hard direction slice and class-balance gates.
  3. The next repair should target public trade/no-trade calibration under centered margin, especially FLAT coverage and trade hard-pred thresholding. Do not move to IQL and do not extend epochs on this red recipe.

## 2026-07-16 Source Update - Max-Normalized Public Margin Direction Composition

- After `SMART_SEQ520_XAU_SMOKE_MARGINCENTER_E6_20260716`, inspection found the remaining structural issue in centered margin composition:
  - mean-centering removed common-mode side bias, but side confidence could still move a trade class over FLAT on no-trade rows;
  - the condition for FLAT to win was still dependent on side magnitude, so public side-choice could still interfere with public trade/no-trade.
- Implemented and committed `48c539f4 Require XAU max-normalized public margin composition`.
  - Added `ENTRY_HIER_PUBLIC_DIRECTION_COMPOSITION=margin_maxnorm`.
  - In `margin_maxnorm` mode, public side logits are max-normalized per row before composition:
    - `LONG=public_trade_logit+(public_side_long_logit-max(side_logits))`
    - `SHORT=public_trade_logit+(public_side_short_logit-max(side_logits))`
    - `FLAT=-public_trade_logit`
  - This makes the strongest side contribution exactly `0`, so side-choice can decide LONG versus SHORT but cannot make a negative trade/no-trade logit beat FLAT.
  - Strict smart XAU repair now requires `margin_maxnorm`; older `margin` and `margin_centered` values remain explicit legacy/non-strict values, not valid XAU repair contracts.
  - Bundle loader, audit/readiness/sweep/wrappers/rebuild, trainer metadata, failure evidence, and tests all carry the new contract.
  - This is not fallback. It is a hard model-topology contract.
- Validation before smoke:
  - `python3 -m py_compile` passed for changed Python modules.
  - `bash -n` passed for changed shell wrappers.
  - `git diff --check` passed.
  - Deterministic model tests passed, including proof that max-normalization preserves the FLAT threshold under extreme side bias.
  - Focused XAU/entry/readiness pytest suite passed.
  - Broader XAU/entry/replay/readiness suite passed, excluding only the two stale legacy-import tests that reference inactive `gx1.execution` modules.
  - Pre-commit hook passed during commit.
- Clean-git post-commit gates passed:
  - `scripts/entry_next_edge_control.sh smart-smoke-readiness --quiet`
  - `scripts/entry_next_edge_control.sh smart-trainability-readiness --quiet`
  - `scripts/entry_next_edge_control.sh smart-smoke-train-enablement --vedtak SMART_SEQ520_XAU_SMOKE_MARGINMAXNORM_ENABLEMENT_20260716 --epochs 6 --batch-size 64 --quiet`
- Ran one bounded smoke and stopped it immediately after epoch `1` validation because it was hard-red:
  - Vedtak: `SMART_SEQ520_XAU_SMOKE_MARGINMAXNORM_E6_20260716`.
  - Pre-train manifest was `/home/andre2/GX1_DATA/reports/entry_foundation_smoke_train_manifests_20260628_v1/ENTRY_FOUNDATION_SMOKE_TRAIN_RUN_MANIFEST_20260716T111237Z.json`, then deleted after manual stop because the run was aborted and stale.
  - Intended bundle dir was not created: `/home/andre2/GX1_DATA/runs/FASE2B_REGIME_V4_20260605/v10_6yr_rebuild_20260628_foundation_seq146/v10_entry_smart_seq520_smoke_20260716T111237Z`.
  - No matching fresh memmap/tmp dir was found.
  - No failure sidecar was written because the process was intentionally interrupted during epoch `2` train before final trainer failure handling.
  - Epoch `1`: `dir_acc=0.330729`, `balance_guard_ok=0`, `slice_contract_ok=0`, `33` slice failures (`13` accuracy, `20` pred-rate), `direction_slice_ckpt_score=-2.076322`.
  - Epoch `1` public pred rates: LONG `0.789714`, SHORT `0.210286`, FLAT `0.000000`; labels were LONG `0.322917`, SHORT `0.332031`, FLAT `0.345052`.
  - Epoch `1` hierarchy evidence: `trade_pred=1.000000`, `trade_prob=0.506023`, `trade_prob_label_flat=0.505870`, `side_pred_long_edge=0.746521`, `side_acc_edge=0.511928`.
  - Interpretation: max-normalized composition is the correct hard separation of side-choice from trade/no-trade topology, but it did not fix the learned hard trade/FLAT surface. The model still predicts trade for every validation row and gives zero FLAT coverage. This is not a candidate.
- Post-stop cleanup/resource state:
  - No transformer train/eval process running.
  - Aborted pre-train manifest deleted.
  - No bundle dir and no fresh memmap/tmp dir existed.
  - `/home/andre2/GX1_DATA`: about `837G` free; RAM about `38GiB` available, swap `0B`.
- Next action:
  1. Do not rerun `SMART_SEQ520_XAU_SMOKE_MARGINMAXNORM_E6_20260716` unchanged.
  2. Candidate/replay/IQL/shadow/live remain closed until a fresh XAU transformer bundle passes hard direction slice and class-balance gates.
  3. The next repair must change the learning formulation or staging for public trade/no-trade hard calibration. The topology now prevents side logits from forcing trade, so the remaining failure is the trade/FLAT learned threshold itself, not IQL.

## 2026-07-16 Source Update - Public Trade Direction-Margin Bridge

- After `SMART_SEQ520_XAU_SMOKE_MARGINMAXNORM_E6_20260716`, the active failure was isolated further:
  - `margin_maxnorm` correctly prevents side-choice from forcing a trade class over FLAT when public trade/no-trade is negative;
  - however, `head_public_trade` started nearly row-constant and slightly positive, so hard public direction still became trade on almost every validation row;
  - the smoke dataset anchor probabilities were uniform across LONG/SHORT/FLAT, so the public trade/FLAT head had too little row-varying signal at epoch 1.
- Implemented and committed `94b568c0 Require XAU public trade direction-margin bridge`.
  - Added `ENTRY_HIER_PUBLIC_TRADE_DIR_MARGIN_BRIDGE=1` with strict XAU defaults `SCALE=0.50`, `CAP=0.25`.
  - The model now adds a bounded, model-native row-varying bridge into `public_trade_logit` before public direction composition:
    - `bridge = mean(raw_direction_logits[LONG], raw_direction_logits[SHORT]) - raw_direction_logits[FLAT]`
    - `public_trade_logit += cap * tanh(scale * bridge / cap)`
  - The bridge feeds only trade-vs-FLAT calibration. It does not choose LONG versus SHORT, does not use labels at inference, and does not open a live rule path.
  - Strict smart XAU repair now requires the bridge enabled with scale in `[0.25, 1.00]` and cap in `[0.05, 0.50]`; wrappers/readiness/manifest/sweep/rebuild, bundle load, trainer metadata, failure evidence, audit, and tests all carry the contract.
  - Also fixed a latent trainer signature bug where `_build_active_head_names(...)` was already called with `hierarchical_public_direction_composition` but the function signature did not accept it.
  - This is not fallback. It is a hard model-topology contract.
- Validation before smoke:
  - `python3 -m py_compile` passed for changed Python modules.
  - `bash -n` passed for changed shell wrappers.
  - `git diff --check` passed.
  - Diff grep confirmed no new fallback implementation text in the code changes.
  - Focused XAU/entry/readiness pytest suite passed:
    - model bridge determinism and maxnorm composition tests;
    - trainer defaults;
    - smoke/candidate wrappers;
    - rebuild contract;
    - smoke enablement/readiness/trainability;
    - sweep lint;
    - bundle audit;
    - candidate/replay readiness.
  - Pre-commit hook passed during commit.
- Clean-git post-commit gates passed:
  - `scripts/entry_next_edge_control.sh smart-smoke-readiness --quiet`
  - `scripts/entry_next_edge_control.sh smart-trainability-readiness --quiet`
  - `scripts/entry_next_edge_control.sh smart-smoke-train-enablement --vedtak SMART_SEQ520_XAU_SMOKE_DIRMARGINBRIDGE_ENABLEMENT_20260716 --epochs 6 --batch-size 64 --quiet`
- Ran one bounded smoke and stopped it immediately after epoch `1` validation because it was still hard-red:
  - Vedtak: `SMART_SEQ520_XAU_SMOKE_DIRMARGINBRIDGE_E6_20260716`.
  - Pre-train manifest was `/home/andre2/GX1_DATA/reports/entry_foundation_smoke_train_manifests_20260628_v1/ENTRY_FOUNDATION_SMOKE_TRAIN_RUN_MANIFEST_20260716T113437Z.json`, then deleted after manual stop because the run was aborted and stale.
  - Intended bundle dir was not created: `/home/andre2/GX1_DATA/runs/FASE2B_REGIME_V4_20260605/v10_6yr_rebuild_20260628_foundation_seq146/v10_entry_smart_seq520_smoke_20260716T113437Z`.
  - No matching fresh memmap/tmp dir was found.
  - Epoch `1`: `dir_acc=0.366536`, `balance_guard_ok=0`, `slice_contract_ok=0`, `28` slice failures (`10` accuracy, `18` pred-rate), `direction_slice_ckpt_score=-1.527204`.
  - Epoch `1` public pred rates: LONG `0.561198`, SHORT `0.412760`, FLAT `0.026042`; labels were LONG `0.322917`, SHORT `0.332031`, FLAT `0.345052`.
  - Epoch `1` hierarchy evidence: `trade_pred=0.986328`, `trade_prob=0.514159`, `trade_prob_label_flat=0.513837`, `side_pred_long_edge=0.578529`, `side_acc_edge=0.556660`.
  - Interpretation: the bridge improved the failure signature versus maxnorm alone (`dir_acc` up from `0.330729` to `0.366536`, FLAT coverage up from `0.000000` to `0.026042`, side edge less collapsed), but it is still not close to the hard class-balance/slice contract. This is progress in diagnosis/topology, not a candidate.
- Post-stop cleanup/resource state:
  - No transformer train/eval process running.
  - Aborted pre-train manifest deleted.
  - No bundle dir and no fresh memmap/tmp dir existed.
  - `/home/andre2/GX1_DATA`: about `837G` free; RAM about `38GiB` available, swap `0B`.
- Next action:
  1. Do not rerun `SMART_SEQ520_XAU_SMOKE_DIRMARGINBRIDGE_E6_20260716` unchanged.
  2. Candidate/replay/IQL/shadow/live remain closed until a fresh XAU transformer bundle passes hard direction slice and class-balance gates.
...
## 2026-07-16 Update: Public Side Direction-Margin Bridge

- Source repair committed:
  - `8b139dec Require XAU public side direction-margin bridge`
  - Added strict XAU `ENTRY_HIER_PUBLIC_SIDE_DIR_MARGIN_BRIDGE=1`, scale `0.50`, cap `0.25`.
  - The bridge adds capped, centered raw LONG/SHORT direction-logit separation into `public_side_logits` before hierarchy public direction composition.
  - This is model-internal signal composition, not a fallback and not a live hand-rule.
- Contract/gate coverage:
  - Model config/forward proof fields added.
  - Train recipe, bundle metadata, bundle loader, smoke/candidate wrappers, 6yr rebuild wrapper, smoke/trainability readiness, smoke enablement, candidate/replay readiness, sweep linter, and smoke bundle audit all require the side bridge for strict XAU repair.
- Validation before smoke:
  - `python3 -m py_compile` on changed Python files: PASS.
  - `bash -n` on changed wrappers: PASS.
  - `git diff --check`: PASS.
  - Diff fallback search: no new `fallback` hits.
  - Focused pytest suite: PASS.
  - `smart-smoke-readiness --quiet`: PASS.
  - `smart-trainability-readiness --quiet`: PASS.
  - `smart-smoke-train-enablement --vedtak SMART_SEQ520_XAU_SMOKE_SIDEMARGINBRIDGE_ENABLEMENT_20260716 --epochs 6 --batch-size 64 --quiet`: PASS.
- Smoke attempted:
  - `SMART_SEQ520_XAU_SMOKE_SIDEMARGINBRIDGE_E6_20260716`
...
- Epoch `1` evidence:
  ...
  - Interpretation: side bridge broke the previous side-long collapse in epoch 1 and restored balanced LONG/SHORT side behavior, but slice contract still failed.
- Epoch `2` hard-red evidence:
  ...
  - Interpretation: the bridge is directionally useful at initialization/epoch 1, but training dynamics pull the public side head back into SHORT starvation by epoch 2. This is now a staging/dynamics problem, not proof that input is missing.
- Cleanup/resource state:
  - Stopped with Ctrl-C after epoch `2` evidence; no transformer train/eval process remains.
  - Pre-train manifest `ENTRY_FOUNDATION_SMOKE_TRAIN_RUN_MANIFEST_20260716T121829Z.json` deleted.
  - Bundle dir `v10_entry_smart_seq520_smoke_20260716T121829Z` was not created.
  - No fresh memmap/tmp dir found.
  - `/home/andre2/GX1_DATA`: about `837G` free; RAM about `37GiB` available; swap `0B`; GPU idle.
- Next action:
  1. Do not rerun `SMART_SEQ520_XAU_SMOKE_SIDEMARGINBRIDGE_E6_20260716` unchanged.
  2. Keep the public side direction-margin bridge contract; it improved epoch 1 and removed the previous immediate `side_pred_long_edge≈0.97` collapse.
