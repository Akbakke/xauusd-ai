...
> clean preflight passes and the operator explicitly reauthorises it.**
...
completed under the CUDA guard and published an immutable bundle. Its later
CPU-only post-run audit is `FAIL` on narrow smoke-quality evidence; a separate
current V9 candidate recipe/gate is dry-run validated only and cannot change
TEST or execution authority.
...
> this new policy.  The 20-minute guard stopped its first window normally at
> 576 partial TRAIN batches, with resumable state slots written.  A fresh
> process then reloaded that exact state, started at batch 577 and durably
> checkpointed batch 640 before a controlled stop.  It did not reach first VAL
...
> 9,655 MiB in the first guard window; the resume canary stayed below the
...
guard exited normally (not a thermal brake) at 65 C / 211.77 W / 8,751 MiB, but
the old movement proof named a static Exit branch that neither public episode
forward executes. Commit `a77ebb6e` rebinds the proof to active
episode-native groups. The fresh repeat then completed the same full preflight,
four optimizer steps and validation under the one-second 220 W process stop
